...
A la comunidad universitaria, y en especial al Grupo de Inteligencia Artificial Aplicada, conmigo han ido mucho más allá de lo necesario. Atesoro increíbles recuerdos y experiencias que me acomparán durante toda mi vida.
...
La importancia de la interpretabilidad de los modelos radica en que, aunque tengamos modelos con métricas perfectas, sólo la intepretabilidad de los resultados permite que las personas confíen en las respuestas; por ejemplo en el campo sanitario es tan importante conocer el porqué de una enfermedad como saber qué enfermedad tiene un paciente. En segundo lugar, recurrir a modelos (ya sean IA o de cualquier tipo) para dar solución a cierto problema (por ejemplo clasificar si un anuncio aumentará el número de clicks) solo permite responder al \textbf{qué} (aumento de clicks) pero no al \textbf{porqué} (qué elementos del anuncio provocan mayores clicks), y en muchas ocasiones es más importante el \textbf{porqué}. Otras razones por las que la interpretabilidad es necesaria son:
\begin{itemize}
    ...
    \item Encontrar un significado a los fenómenos que no entedemos. Por ejemplo \textit{¿Por qué Amazon me recomienda estos productos?}
          \begin{figure}[H]
              \ffigbox[\FBwidth] {
    ...
    \item Detectar sesgo: para poder reconducir o prevenir que el modelo tome deciones sesgadas, tomando decisiones que serían consideradas injustas.
    \item Aceptación social, para que el modelo pueda ser usado sin causar malestar.
    \item Encontrar un lenguaje común. Por ejemplo un robot limpiador puede quedarse parado porque su motor paso a paso no puede girar al menos a 20 rpm, la señal real es \textit{el pin 12 necesita al menos 200mA, está por encima del límite programado}; la interpretración con lenguaje común debería ser \textit{estoy atascado porque no consigo girar la rueda}.
...
\begin{itemize}
    \item Son contrastivas \cite{Lipton1990}. No solo queremos conocer porqué el modelo dió un resultado \textit{A}, sino también porqué \textit{A} y no otro resultado \textit{A'}. La explicaciones que son contrastivas nos dan una respuesta de las diferencias que han provocado una respuesta \textit{A} a diferencia de una respuesta \textit{A'}. La explicaciones que son contrastivas dependen totalmente de la aplicación que se le de al modelo, por ejemplo un modelo puede predecir que un paciente tiene un melanoma. Desde el punto del vista del médico, la explicación contrastiva es \textit{¿qué diferencia a este paciente de otro paciente con características similares?}, pero desde el punto de vista del paciente la explicación contrastiva sería \textit{¿qué valores debería tener distintos para que no hubiese predicho melanoma?}. El médico puede probar otras terapias, mientras que el paciente puede fijarse en otros pacientes para copiar su estilo de vida; aunque el modelo es el mismo la explicación contrastiva es distinta.
...
Los métodos de XAI son diversos y permiten atacar los problemas de explicabilidad desde distintos ángulos, dependendiendo de las necesidades del problema. Según \cite{molnar2025, Kamath2021} los problemas de explicabilidad se pueden acotar según:
\begin{itemize}
...
\begin{enumerate}
    \item Validez. Si respecto a una instancial original $x$ y un valor objetivo $y*$, se genera una nueva instancia $x'$ tal que efectivamente se llega al resultado deseado.
    ...
    \item Causalidad. Es una paso posterior a que un contrafactual sea plausible y accionable, el contrafactual debería ser capaz de mantener las relaciones que existen entre las variables. Por ejemplo el tipo de interés de un crédito se ve afectado por los años de amortización; en una deuda de tipo \textit{unsecured}, a mayor tiempo, mayor interés, \textit{ceteris paribus}, y no sería causal una explicación contrafactual en la que se baje el tiempo de la deuda y se mantuviese el interés. Es una de las grandes incógnitas de la literatura el cómo incorporar causalidad a las explicaciones contrafactuales.
...
\begin{enumerate}
    ...
    \item Justos. Es quizás lo más defícil de conseguir, pues implica que variar ciertos atributos de la explicación no invalida la validez de la explicación. Por ejemplo, en un sistema de crédito el algoritmo de generación debería ser capaz de generar la misma explicación contrafactual independientemente de la raza de la persona.
\end{enumerate}
...
\begin{itemize}
    \item Método de generación
          \begin{itemize}
              ...
              \item Por métodos de optimización. Es el método que usa la mayoría de algoritmos. Primero se defina una o varias funciones de pérdida, y luego se usa un algoritmo de optimización para generar los contrafactuales.
...
Concretando aún más en el dominio de las series temporales, los algoritmos intentan explotar la estructura temporal de los datos para aumentar la plausibilidad de las explicaciones. En \cite{Ates2021} describen CoMTE, el cual basa su procedimiento en las instancias "distractoras", instancias reales del dataset a las cuales mediante un algoritmo tipo \textit{greedy} se van intercambiando las features hasta que cambia la clasificación; de todos los cambios estudiados, se busca el que menos cambiaos tenga respecto a la instancia original. En \cite{Delaney2021} implementan \textit{Native Guide}, el cual se basa encontrar la instancia más cercana que cambie la salida del modelo (también conocida como \textit{nearest-unlike neighbour}); se usa una distancia manhattan con búsqueda de tipo \textit{greedy}. En \cite{Filali2022} detallan Timex, donde minimizan 3 objetivos simultáneos, diferencia de objetivos, diferencia respecto a la instancia original y (como novedad) la diferencia entre baricentros de las series, luego generan un nuevo dataset con distintas predicciones que luego se usa para encontrar el contrafactual vía distancia manhattan. En \cite{Bahri2024} presentan DiscoX, en el que usan una medida basada en series de discordia (una serie temporal de discordia es una subsecuencia que tiene la mayor distancia respecto a otra $T$, es decir una serie de discordia en $T$ en referencia a $T'$ tiene forma similar a $T$ pero forma muy distinta a $T'$), y construyen un nuevo dataset que se usa para buscar el contrafactual mediante perturbaciones. En \cite{Lang2023} describen SPARCE, un método de generación que se centra más en generar contrafactuales variados, para ello entrenan una red GAN que luego optimizan por descenso de gradiente; la función de pérdida combina 3 términos, uno de validez, uno de proximidad y otro de variedad. En \cite{Qi2024} proponen TIME-CF, que usa shapelets (segmentos de la serie con formas características); la algoritmo de generación de contrafactuales trata de sustituir los shapelets (generados con una red GAN) hasta hallar el contrafactual. En \cite{Wang2024} exponen Glacier, donde primero entrenan un auto-encoder, sobre el que aplican descenso del gradiente minimizando una función de pérdida que combina similaridad con plausibilidad; el resultado es un único contrafactual. Continuando con \cite{Xiangyu2025}, implementan CFWoT (Counterfactuals Without Traning data), en el que mediante aprendizaje por refuerzo se varia la instancia original hasta que se encuentra un estado en el que ha llegado al objetivo contrafactual; se reportan los contrafactuales de menor a mayor distancia respecto a la instancia original.
...
\begin{itemize}
    ...
    \item \textbf{Fase 5: Redacción de la memoria y revisión final.} La última fase se dedicó a la elaboración del documento \LaTeX completo. Esta fase se extendió desde enero de 2026 hasta la entrega final en septiembre de 2026. Confome se materializaban ciertos resultados del algoritmo se plasmaban directamente en la memoria.
\end{itemize}
