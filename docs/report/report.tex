...
\begin{itemize}
    ...
    \item Los errores sean independientes de los predictores.
...
\end{equation}
En esta especificación, los predictores se observan $h$ periodos antes que la variable que se desea pronosticar. Esto tiene una ventaja práctica importante: al proyectar el modelo más allá del final de la muestra, los valores de entrada ya son conocidos y no es necesario predecir simultáneamente todos los regresores. Además, esta formulación suele ser más verosímil ya que muchos efectos no se materializan de forma instantánea, sino con cierto retraso \cite{Anderson1994}. Cuando incluso esta estrategia resulta insuficiente, puede recurrirse a análisis por escenarios, fijando trayectorias plausibles para los predictores y evaluando cómo cambian las predicciones del modelo bajo cada una de ellas.
...
\end{equation}
donde $T$ es el tamaño muestral, $\bar{x}$ es la media observada del predictor, $s_x$ es su desviación típica y $\hat{\sigma}_e$ es el error estándar residual. Para niveles de cobertura distintos basta con reemplazar el valor $1.96$ por el cuantil correspondiente; por ejemplo, un intervalo aproximado al $80\%$ se obtiene usando $1.28$. Esta expresión muestra además que la incertidumbre de la predicción crece cuando el valor del predictor se aleja de su media muestral, lo que refleja que el modelo predice con mayor precisión en regiones del espacio de entrada bien representadas por los datos observados \cite{Hyndman2021}.

\textbf{Forma matricial.} En problemas con múltiples observaciones y varios predictores, resulta conveniente escribir el modelo en forma matricial. Sea
$$
...
$$
y sea además la matriz de datos $X$ de dimensión $T \times (k+1)$, donde la primera columna es un vector de unos para el intercepto y las siguientes columnas contienen los valores de los predictores para cada observación:
$$
X =
\begin{pmatrix}
...
\end{equation}
donde se asume que $\mathbb{E}[\boldsymbol{\varepsilon}] = \mathbf{0}$ y $\operatorname{Var}(\boldsymbol{\varepsilon}) = \sigma^2 I$. La matriz $X$ tiene $T$ filas, una por observación, y $k+1$ columnas, correspondientes al intercepto y a los $k$ predictores incluidos en el modelo.
...
\end{equation}
donde $e_t$ es el residuo obtenido al ajustar el modelo con las $T$ observaciones. Esta identidad evita tener que reestimar el modelo $T$ veces para evaluar el error de predicción fuera de muestra \cite{Hyndman2021}.

\textbf{Predicción e intervalos en forma matricial.} Sea $\mathbf{x}_*$ un vector fila que contiene los valores de los predictores, con el mismo formato que las filas de $X$, para los que se desea obtener una predicción. Entonces, la predicción puntual viene dada por
\begin{equation}
...
\end{equation}
Este intervalo incorpora tanto la incertidumbre asociada al término de error como la derivada de estimar los coeficientes del modelo. Sin embargo, no recoge la incertidumbre adicional procedente de posibles errores en los valores futuros de los predictores. Por ello, cuando $\mathbf{x}_*$ también debe pronosticarse, el intervalo anterior tiende a infraestimar la incertidumbre total \cite{Hyndman2021}.

\paragraph{Clasificación}\mbox{}

La clasificación de series temporales trata de asignar una etiqueta (\textit{label}) a la serie. Este tipo de problemas se considera de información completa, ya que se dispone de la serie completa para tomar la decisión. Por ejemplo, clasificar una serie temporal de temperatura como \textit{cómoda} o \textit{incómoda}. Entra dentro de la clase de problemas supervisados, ya que se dispone de un conjunto de entrenamiento con series temporales etiquetadas. La teoría estadística de la clasificación de series temporales se basa en la teoría de la clasificación supervisada clásica (métodos como \textit{clasificadores logísticos}, \textit{knn}, etc.), pero adaptada a la naturaleza secuencial de los datos.
...
\begin{definition}[Error de predicción por objetivo]
    ...
    Esta definición se instanciará de dos formas en lo que sigue: con $\boldsymbol{\theta}^{(d)} = \mathbf{w}^{(d)}$ para resultados relativos a un modelo lineal genérico, y con $\boldsymbol{\theta}^{(d)} = \boldsymbol{\tau}^{(d)}$ para comparar CausalModel frente a MCO respecto del \emph{efecto total}.
\end{definition}

\subsubsection{Sesgo dinámico por bloqueo de mediación}

En esta formulación del sesgo del MCO no se identifica con un error aditivo arbitrario respecto de un coeficiente directo, sino con el \textbf{sesgo por bloqueo de trayectorias mediadas} en sistemas dinámicos. Para un horizonte $h+1$ se define el sesgo matricial del MCO plenamente ajustado por
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{LinearRegression} (MCO plenamente ajustado, pero sin usar el DAG):
...
\begin{proof}[Derivación]
    ...
    Para el sesgo del MCO plenamente ajustado, aplicamos el teorema anterior de sesgo de mediación en el mismo horizonte:
...
\begin{corollary}[Caso AR(2) escalar]
    ...
    $$\tau^{(2)} = \phi_2 + \phi_1^2,$$
    y el sesgo del MCO plenamente ajustado es
