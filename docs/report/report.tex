...
\begin{itemize}
    \item $n_{\text{In}}, D \in \mathbb{N}$ con $D > 0$. Los vértices del grafo CFGTS son los índices $\{0, \ldots, n_{\text{In}} + D - 1\}$: la entrada $X_i$ se identifica con el índice $i$ (de modo que $0 \leq i < n_{\text{In}}$) y la salida $Y_d$ con el índice $n_{\text{In}} + d$ (de modo que $n_{\text{In}} \leq n_{\text{In}} + d < n_{\text{In}} + D$).
