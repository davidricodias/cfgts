...
\pdfgentounicode=1
\pdfglyphtounicode{a}{0062}
\pdfglyphtounicode{b}{0063}
\pdfglyphtounicode{c}{0064}
\pdfglyphtounicode{d}{0065}
\pdfglyphtounicode{e}{0066}
\pdfglyphtounicode{f}{0067}
\pdfglyphtounicode{g}{0068}
\pdfglyphtounicode{h}{0069}
\pdfglyphtounicode{i}{006A}
\pdfglyphtounicode{j}{006B}
\pdfglyphtounicode{k}{006C}
\pdfglyphtounicode{l}{006D}
\pdfglyphtounicode{m}{006E}
\pdfglyphtounicode{n}{006F}
\pdfglyphtounicode{o}{0070}
\pdfglyphtounicode{p}{0071}
\pdfglyphtounicode{q}{0072}
\pdfglyphtounicode{r}{0073}
\pdfglyphtounicode{s}{0074}
\pdfglyphtounicode{t}{0075}
\pdfglyphtounicode{u}{0076}
\pdfglyphtounicode{v}{0077}
\pdfglyphtounicode{w}{0078}
\pdfglyphtounicode{x}{0079}
\pdfglyphtounicode{y}{007A}
\pdfglyphtounicode{z}{0061}
\pdfglyphtounicode{A}{0042}
\pdfglyphtounicode{B}{0043}
\pdfglyphtounicode{C}{0044}
\pdfglyphtounicode{D}{0045}
\pdfglyphtounicode{E}{0046}
\pdfglyphtounicode{F}{0047}
\pdfglyphtounicode{G}{0048}
\pdfglyphtounicode{H}{0049}
\pdfglyphtounicode{I}{004A}
\pdfglyphtounicode{J}{004B}
\pdfglyphtounicode{K}{004C}
\pdfglyphtounicode{L}{004D}
\pdfglyphtounicode{M}{004E}
\pdfglyphtounicode{N}{004F}
\pdfglyphtounicode{O}{0050}
\pdfglyphtounicode{P}{0051}
\pdfglyphtounicode{Q}{0052}
\pdfglyphtounicode{R}{0053}
\pdfglyphtounicode{S}{0054}
\pdfglyphtounicode{T}{0055}
\pdfglyphtounicode{U}{0056}
\pdfglyphtounicode{V}{0057}
\pdfglyphtounicode{W}{0058}
\pdfglyphtounicode{X}{0059}
\pdfglyphtounicode{Y}{005A}
\pdfglyphtounicode{Z}{0041}
\pdfglyphtounicode{zero}{0031}
\pdfglyphtounicode{one}{0032}
\pdfglyphtounicode{two}{0033}
\pdfglyphtounicode{three}{0034}
\pdfglyphtounicode{four}{0035}
\pdfglyphtounicode{five}{0036}
\pdfglyphtounicode{six}{0037}
\pdfglyphtounicode{seven}{0038}
\pdfglyphtounicode{eight}{0039}
\pdfglyphtounicode{nine}{0030}
...
% Colores inspirados en el tema claro por defecto de VS Code (Light+)
\definecolor{vscBackground}{RGB}{255,255,255}
\definecolor{vscBorder}{RGB}{229,229,229}
\definecolor{vscKeywordControl}{RGB}{175,0,219}
\definecolor{vscKeywordStorage}{RGB}{0,0,255}
\definecolor{vscString}{RGB}{163,21,21}
\definecolor{vscComment}{RGB}{0,128,0}
\definecolor{vscLineNumber}{RGB}{160,160,160}

% definimos un estilo de listings (paleta VS Code Light+)
\lstdefinestyle{estilo}{ frame=single,
	framerule=0.4pt,
	...
	keywordstyle=\color{vscKeywordControl},
	emph={def, class, None, True, False},
	emphstyle=\color{vscKeywordStorage},
	stringstyle=\ttfamily\color{vscString},
...
\begin{titlepage}
    \begin{sffamily}
        ...
        \begin{center}
            ...
            \end{Large}
            {\Huge ``Propuesta de algoritmo de explicación contrafactual para el análisis de modelos predictivos de series temporales''}\\
            \vspace*{0.5cm}
            \rule{10.5cm}{0.1mm}\\
            \vspace*{0.9cm}
            {\LARGE José David Rico Días}\\
            \vspace*{1cm}
            \begin{Large}
                Tutor\\
                Antonio Berlanga de Jesús\\
                Colmenarejo, Septiembre 2026\\
            \end{Large}
...
\begin{abstract}
    ...
    El trabajo que aquí se presenta contiene un algoritmo de generación de explicaciones contrafactuales para modelos de regresión en series temporales.

    El algoritmo consiste en 3 fases diferenciadas. La primera fase de generación de contrafactuales encontrados mediante la resolución de problema de optimización no lineal. La segunda fase es la generación de un espacio de cobertura  generado por los valores objetivo en los contrafactuales. Por último se genera un modelo causal sobre el intervalo de cobertura, y se asigna una puntuación a cada contrafactual; la puntuación mide el grado de alineación del contrafactual contra las suposiciones causales que tienen los modelos de series temporales.

    El trabajo incluye la implementación del algoritmo que se ha realizado en Python con código públicamente accesible.

    \textbf{Palabras clave:}
    % Escribir las palabras clave aquí
    Explicaciones contrafactuales, Series temporales, Causalidad, Grafos causales, Inteligencia artificial explicable (XAI), Optimización multi-objetivo, Modelos autorregresivos, Modelos surrogados, Algoritmo
...
Mamá, Ari, Sara, Samuel sois las personas que más me han apoyado, mi familia. Todos hemos luchado para poder continuar mis estudios, y mis éxitos han sido igualmente vuestros éxitos.
A la comunidad universitaria, y en especial al Grupo de Inteligencia Artificial Aplicada, conmigo han ido mucho más allá de lo necesario. Atesoro increíbles recuerdos y experiencias que me acompañarán durante toda mi vida.
...
Una forma de obtener mejor entendimiento a estas preguntas es a través de las explicaciones contrafactuales, las cuales comparan una instancia observada con una alternativa hipotética: \textit{¿qué valores deberían haber sido diferentes para alcanzar un objetivo determinado?}. En las series temporales estas instancias son instantes pasados, que están ordenados y pueden estar relacionadas entre sí. Una explicación contrafactual no tiene porqué respetar la coherencia temporal.

Este TFG detalla e implementa \texttt{CFGTS} (\textit{CounterFactual Generation for Time Series}), un algoritmo para generar y evaluar explicaciones contrafactuales en modelos autorregresivos de series temporales. A partir de un modelo de regresión ajustado, una instancia temporal y un valor objetivo de predicción, el algoritmo busca candidatos que equilibren la aproximación al objetivo y la cercanía a la observación original. Después, construye una región local de cobertura y asigna a cada candidato una puntuación de consistencia causal basada en la estructura temporal de sus variables.
...
\begin{enumerate}
    ...
    \item Diseñar un algoritmo de 3 fases que: genera contrafactuales, genera un espacio acoplado (conjunto de cobertura) y por último puntúa los contrafactuales según su consistencia causal.
    \item Formaliza las condiciones mediante el cual el algoritmo es capaz de encontrar soluciones. Uno de los puntos principales es la demostración de que es necesario puntuar los contrafactuales mediante un grafo, ya que sino se generaría un sesgo de mediación.
    \item Implementa el algoritmo en una biblioteca Python. La biblioteca está disponible para su descarga mediante \texttt{PyPi}.
    \item Evalúa el comportamiento de la propuesta, comparando modelos lineales y no lineales, y contrastando los candidatos generados con un control aleatorio.
\end{enumerate}


\section{Estructura del documento}

El documento se organiza de la siguiente forma: el Capítulo~\ref{cap:motivacion}, Motivación, presenta los casos de uso que originan el problema. A continuación, el Capítulo~\ref{cap:fundamentos}, Fundamentos teóricos, introduce los conceptos de estadística, series temporales, causalidad y explicabilidad necesarios para el desarrollo posterior, y el Capítulo~\ref{cap:estadodelarte}, Estado del arte, revisa los métodos relacionados de generación de contrafactuales y modelos surrogados.

El Capítulo~\ref{cap:analisis}, Análisis del problema, concreta los requisitos de \texttt{CFGTS}. El Capítulo~\ref{cap:propuesta}, Propuesta, desarrolla su formalización, las tres fases del algoritmo y su implementación. Después, el Capítulo~\ref{cap:resultados}, Resultados, describe el protocolo experimental y analiza el comportamiento obtenido. Los capítulos finales examinan el marco regulador (Capítulo~\ref{cap:regulacion}), los impactos del trabajo (Capítulo~\ref{cap:impacto}), la planificación (Capítulo~\ref{cap:planificacion}) y el presupuesto (Capítulo~\ref{cap:presupuesto}), antes de cerrar con las conclusiones (Capítulo~\ref{cap:conclusiones}) y las futuras líneas de investigación (Capítulo~\ref{cap:futuras_lineas}). Los anexos reúnen el glosario, las demostraciones y las declaraciones requeridas.

\chapter{Motivación}
\label{cap:motivacion}

La motivación de este trabajo surge desde dos perspectivas:
\begin{itemize}
    \item Un problema que me encontré durante una etapa de trabajo en \textit{Santander Corporate and Investment Banking}: dados los movimientos diarios en rendimiento de un portfolio de deuda, explicar qué tipo de portfolio tendríamos que haber tenido ayer para que el movimiento hubiese sido cero, en vez del movimiento finalmente observado. Los instrumentos de deuda presentan autocorrelación y correlación cruzada. Es necesario predecir los movimientos de ambos instrumentos, y explicar qué movimientos en los instrumentos de deuda en los días anteriores podrían haber llevado a un movimiento cero en el día actual. Este problema se puede formular como un problema de generación de explicaciones contrafactuales para modelos predictivos de series temporales.
...
\textbf{Variable continua.} Si $X$ es continua, entonces debe existir un $f_X$ tal que
$$
...
Donde $m(x)$ se aproxima mediante una función parametrizada sobre $x$; la más habitual es la lineal:
\begin{equation}
    \hat{y}=a+bx.
...
\subsubsection{Efecto total y mediación}
...
El problema de la caja negra es estudiado en profundidad por el campo de la IA explicable (\textit{XAI}, \textit{Explainable AI}). La XAI permite obtener nuevas interpretaciones sobre los resultados y funcionamientos de modelos, que a priori no son explicables.

La \textit{interpretabilidad} es el grado en el que un ser humano es capaz de entender la causa de una decisión \cite{Miller2019}, y es el término más básico en explicabilidad: una persona es capaz de entender el \textit{porqué} de un resultado. Esta definición además permite dar un grado de interpretabilidad a los modelos, los modelos más interpretables son más fáciles de entender, y viceversa.

La importancia de la interpretabilidad de los modelos radica en que, aunque tengamos modelos con métricas perfectas, solo la interpretabilidad de los resultados permite que las personas confíen en las respuestas; por ejemplo, en el campo sanitario es tan importante conocer el porqué de una enfermedad como saber qué enfermedad tiene un paciente. En segundo lugar, recurrir a modelos (ya sean IA o de cualquier tipo) para dar solución a cierto problema (por ejemplo, clasificar si un anuncio aumentará el número de clics) solo permite responder al \textbf{qué} (aumento de clics) pero no al \textbf{porqué} (qué elementos del anuncio provocan mayores clics), y en muchas ocasiones es más importante el \textbf{porqué}. Otras razones por las que la interpretabilidad es necesaria son:
\begin{itemize}
    \item Curiosidad: la necesidad humana de entender aunque no nos suponga ningún beneficio directo. Por ejemplo saber porqué un modelo clasifica un planeta como habitable o no habitable.
    \item Encontrar un significado a los fenómenos que no entendemos. Por ejemplo \textit{¿Por qué Amazon me recomienda estos productos?}
          \begin{figure}[H]
              \ffigbox[\FBwidth] {
                  \caption[Recomendación y su significado]{Recomendación de Amazon por la compra de un iPad Pro M5. El modelo de recomendación muestra los productos que se compran juntos frecuentemente. La web de Amazon muestra el significado de esa recomendación. Fuente: elaboración propia.}
              }
              {\includegraphics[width=0.75\linewidth]{recomendados-porque.png}}
          \end{figure}

    \item Detectar sesgo: para poder reconducir o prevenir que el modelo tome decisiones sesgadas, tomando decisiones que serían consideradas injustas.
    \item Aceptación social, para que el modelo pueda ser usado sin causar malestar.
    \item Encontrar un lenguaje común. Por ejemplo un robot limpiador puede quedarse parado porque su motor paso a paso no puede girar al menos a 20 rpm; la señal real es \textit{el pin 12 necesita al menos 200mA, está por encima del límite programado}; la interpretación con lenguaje común debería ser \textit{estoy atascado porque no consigo girar la rueda}.
    \item Auditar los modelos, para prevenir fallos sistemáticos. Por ejemplo un modelo de clasificación de cáncer puede tener un 99.9\% de \textit{accuracy} promedio, pero para un tipo de cáncer concreto tener un 20\%.
\end{itemize}

Una \textit{explicación} consiste en una forma superior de interpretabilidad, y es una respuesta a una pregunta de tipo \textbf{porqué}, por ejemplo \textit{¿por qué el alumno tardó tanto en entregar el TFG?}. Las buenas explicaciones cumplen \cite{Miller2019}:
\begin{itemize}
    \item Son contrastivas \cite{Lipton1990}. No solo queremos conocer porqué el modelo dio un resultado \textit{A}, sino también porqué \textit{A} y no otro resultado \textit{A'}. Las explicaciones contrastivas nos dan una respuesta sobre las diferencias que han provocado una respuesta \textit{A} frente a una respuesta \textit{A'}. Las explicaciones contrastivas dependen totalmente de la aplicación que se le dé al modelo; por ejemplo, un modelo puede predecir que un paciente tiene un melanoma. Desde el punto de vista del médico, la explicación contrastiva es \textit{¿qué diferencia a este paciente de otro paciente con características similares?}, pero desde el punto de vista del paciente la explicación contrastiva sería \textit{¿qué valores debería tener distintos para que no hubiese predicho melanoma?}. El médico puede probar otras terapias, mientras que el paciente puede fijarse en otros pacientes para copiar su estilo de vida; aunque el modelo es el mismo, la explicación contrastiva es distinta.
    \item Son selectivas, pues dan los motivos que más han influenciado la decisión del modelo.
    \item Son sociales, dependen del conocimiento y capacidad del receptor, y están hechas para las personas. A un joven ingeniero informático le puedes explicar que la web no funciona porque debido a falta de memoria en los nodos, hay un \texttt{CrashLoopBackoff} en los \texttt{pods}, y ha causado un \textit{retry storm}, pero a un ejecutivo debes explicarle que la caída de la web \textit{se debe a causas ajenas al equipo de ingeniería, y se está trabajando en recuperarla}. Ambas explicaciones son ciertas, pero están adaptadas a la persona que la recibe.
    \item Se centran en lo más sorprendente para el receptor de la explicación, pues las personas se fijan más en las razones sorprendentes que en las comunes.
    \item Son fieles, pues sino no sirven de nada.
    \item Son consistentes con las creencias que tiene el receptor de la explicación. Las explicaciones deben seguir el modelo mental del receptor sobre las causas y los efectos. Una explicación demasiado sorprendente puede ser descartada como incierta.
    \item Se pueden refutar, es decir que si no fuese cierta la explicación se puede comprobar. Por ejemplo para el modelo de publicidad creamos una explicación que dice que dice que el tamaño del título afecta en los clicks; se puede refutar poniendo el tamaño del título igual que el texto principal y observando como bajan los clicks.
\end{itemize}

\begin{figure}[H]
    \ffigbox[\FBwidth] {
        \caption[Niveles de entendimiento de los modelos]{Niveles de entendimiento de los modelos desde el punto de vista de un investigador. Fuente: elaboración propia.}
    }
    {\includegraphics[width=0.85\linewidth]{interpretabilidad.png}}
\end{figure}

Los métodos de XAI son diversos y permiten atacar los problemas de explicabilidad desde distintos ángulos, dependiendo de las necesidades del problema. Según \cite{molnar2025, Kamath2021}, los problemas de explicabilidad se pueden acotar según:
\begin{itemize}
    \item Alcance de la explicabilidad
          \begin{itemize}
              \item Global. Los métodos de alcance global son útiles cuando se busca explicar el comportamiento agregado del modelo, y se provee de métodos que permiten comprender el modelo independientemente de los inputs utilizados. Este tipo de alcance da un entendimiento sobre los mecanismos internos del modelo.
              \item Local. En contraste, el alcance local trata de explicar como un subconjunto de las posibles entradas se comporta una vez entra al modelo para producir la salida. Lo que se busca es comprender cómo se ha generado el output a partir de cierto input, es decir acotando el funcionamiento del modelo a las partes que involucran dicho input.
          \end{itemize}
    \item Etapa de uso del modelo
          \begin{itemize}
              \item Previo al uso de un modelo. Aquí pertenecen las técnicas que solo son aplicables a los datos, y también se le conoce como \textit{EDA}, \textit{Exploratory Data Analysis}. Los métodos de explicabilidad previos al uso del modelo tratan de adquirir un entendimiento de cómo se generan los datos y cómo se relacionan.
              \item Con el modelo entrenado. También se lo conoce como métodos autoexplicativos, y usan la estructura interna del modelo, que suele ser inherentemente explicable, para producir la explicación. Incluye métodos como los árboles de decisión.
              \item Con el modelo produciendo salidas. Se suelen aplicar a modelos considerados cajas negras, como las redes de neuronas, y son aplicables cuando la estructura interna del modelo no permite adquirir un entendimiento.
          \end{itemize}
    \item Conocimiento sobre el modelo
          \begin{itemize}
              \item Específico del modelo. Cuando el procedimiento para adquirir explicabilidad es específico, se trata de explotar cierta estructura o conocimiento que tenemos del modelo para producir explicaciones; esto causa que dichos métodos solo puedan usarse en modelos concretos. Por ejemplo, los métodos de explicabilidad de los árboles de decisión son necesariamente específicos, ya que se debe tener un árbol y no otro tipo de estructura. Son métodos más ventajosos que los agnósticos, porque explotan más información.
              \item Agnóstico al modelo. Son métodos de XAI aplicables a cualquier modelo, y no explotan el funcionamiento del modelo, sino que a través de las entradas, salidas y ejecutar el modelo varias veces, es posible obtener explicaciones. Por ejemplo el método LIME (\textit{Local Interpretable Model-agnostic Explanations}).
          \end{itemize}
\end{itemize}
...
Las explicaciones contrafactuales son predicciones que describen el cambio mínimo a realizar a una instancia para que el modelo prediga un nuevo valor predefinido. Estas predicciones responden a preguntas del tipo \textit{"si X no hubiese ocurrido, Y no habría ocurrido"}, como por ejemplo \textit{"si hubiese tenido mayor salario, me habrían concedido el préstamo"}. Las explicaciones contrafactuales son predicciones sobre mundos hipotéticos, ya que la instancia original es el suceso que en realidad sí ocurrió. Las explicaciones contrafactuales deben ser amigables con los seres humanos, similares a la instancia original, accionables y variadas \cite{molnar2025}.
...
\begin{enumerate}
    \item Validez. Si respecto a una instancia original $x$ y un valor objetivo $y*$, se genera una nueva instancia $x'$ tal que efectivamente se llega al resultado deseado.
    \item Minimalidad. No deberían cambiarse atributos innecesariamente. Por ejemplo si dos explicaciones contrafactuales $c_1$, $c_2$ dicen que deberías llegar a 25000€ de salario para obtener un crédito, pero $c_2$ además dice que te deberías mudar, se debería preferir $c_1$ por tener menos variaciones.
    \item Similaridad. El contrafactual debería ser lo más cercano a la instancia original. Existe gran debate en la literatura sobre cómo definir la similaridad, aunque la mayoría de métodos se decantan por usar distancias de tipo norma ($L_2$, $L_{\infty}$, etc.)
    \item Plausibilidad. Dada la distribución original de los datos, los contrafactuales deberían poder existir dentro de esa distribución. El problema de saber si una instancia contrafactual es plausible depende de poseer datos que reflejen fidedignamente la distribución real. Además la plausibilidad se ve afectada por la "maldición de la dimensionalidad".
    \item Poder discriminante. Un contrafactual debería cambiar las variables que más afectan a la decisión final en orden. Es decir, si un sistema de créditos asigna como variable más importante el salario, el contrafactual debería reflejar esta importancia cambiando más el salario que el resto de variables.
    \item Accionabilidad. Todos los contrafactuales tienen como objetivo personas, por ello es necesario proveer contrafactuales que puedan dar pasos a los usuarios para cambiar el resultado del modelo. Por ejemplo un contrafactual podría decir que siendo más joven te habrían dado el crédito, pero esa explicación no es accionable para un usuario.
    \item Causalidad. Es un paso posterior a que un contrafactual sea plausible y accionable: el contrafactual debería ser capaz de mantener las relaciones que existen entre las variables. Por ejemplo, el tipo de interés de un crédito se ve afectado por los años de amortización; en una deuda de tipo \textit{unsecured}, a mayor tiempo, mayor interés, \textit{ceteris paribus}, y no sería causal una explicación contrafactual en la que se baje el tiempo de la deuda y se mantuviese el interés. Es una de las grandes incógnitas de la literatura cómo incorporar causalidad a las explicaciones contrafactuales.
    \item Diversidad. Es deseable que se generen varias explicaciones válidas, de tal forma que el usuario objetivo de las explicaciones tenga un mejor entendimiento. Es una pregunta abierta de la literatura el cómo ordenar los contrafactuales que ya son diversos.
\end{enumerate}

Así mismo los algoritmos que generan las explicaciones contrafactuales deberían cumplir ser:
\begin{enumerate}
    \item Eficientes. Deberían generar la explicación en un tiempo razonable para el usuario
    \item Estables. Dados dos modelos que den predicciones similares (por ejemplo un \textit{Knn} y \textit{CatBoost}), un mismo algoritmo de generación de contrafactuales debería dar explicaciones parecidas para ambos modelos.
    \item Consistentes. Un algoritmo de generación de contrafactuales es consistente si los cambios que se realizan en el modelo original (por ejemplo por entrenar con nuevos datos) refleja los cambios del modelo de forma fidedigna. Por ejemplo si el modelo de aceptación de crédito ahora necesita un mínimo de 30000€ para poder dar un crédito, el algoritmo de generación de explicaciones debería tener en cuenta el nuevo mínimo.
    \item Justos. Es quizás lo más difícil de conseguir, pues implica que variar ciertos atributos de la explicación no invalida la validez de la explicación. Por ejemplo, en un sistema de crédito el algoritmo de generación debería ser capaz de generar la misma explicación contrafactual independientemente de la raza de la persona.
\end{enumerate}
...
\begin{itemize}
    \item Método de generación
          \begin{itemize}
              \item Por fuerza bruta. Es uno de los primeros métodos que se desarrollaron, y consiste en una búsqueda de tipo "grid search", probando todas las posibilidades de la distribución hasta encontrar una instancia que cambie el valor objetivo.
              \item Por métodos de optimización. Es el método que usa la mayoría de algoritmos. Primero se definen una o varias funciones de pérdida, y luego se usa un algoritmo de optimización para generar los contrafactuales.
              \item Por métodos heurísticos. Son mucho más eficientes que los métodos de optimización, con la contrapartida de no presentar contrafactuales con buenas propiedades. Suelen presentarse en algoritmos específicos al modelo, también en problemas con tipos de datos más concretos (como texto).
          \end{itemize}
    \item Información sobre el modelo original
          \begin{itemize}
              \item Agnósticos al modelo, si el algoritmo de generación se puede aplicar a cualquier modelo.
              \item Específicos del modelo, si el algoritmo de generación sólo puede aplicarse a un modelo concreto.
          \end{itemize}
    \item Según el uso de los datos de entrenamiento
          \begin{itemize}
              \item Exógenos a los datos. Este tipo de algoritmos no usan las instancias de los datos de entrenamiento del modelo original para generar los contrafactuales. Pueden generar contrafactuales que estén fuera de los datos de entrenamiento
              \item Endógeno a los datos. Cuando el algoritmo es endógeno a los datos, buscará los contrafactuales en el propio dataset de entrenamiento. Este tipo de algoritmos están muy relacionados con los de generación de instancias prototípicas.
          \end{itemize}
\end{itemize}
...
Respecto a los algoritmos de generación de contrafactuales de clasificación, encontramos en el estado del arte
métodos que intentan balancear las propiedades antes expuestas. En \cite{Marzari2024} se centran en la robustez de las explicaciones contrafactuales en modelos de redes de neuronas, generando los contrafactuales cambiando los parámetros de la red de neuronas. En \cite{Mothilal2020} muestran \texttt{DiCE}, en el que principalmente se añade un término de pérdida que penaliza la falta de diversidad, y en modelos tipo caja negra realizan la búsqueda de los contrafactuales con un algoritmo genético.
% Clasificación en series temporales
Concretando aún más en el dominio de las series temporales, los algoritmos intentan explotar la estructura temporal de los datos para aumentar la plausibilidad de las explicaciones. En \cite{Ates2021} describen CoMTE, el cual basa su procedimiento en las instancias "distractoras", instancias reales del conjunto de datos a las cuales, mediante un algoritmo de tipo \textit{greedy}, se les van intercambiando las características hasta que cambia la clasificación; de todos los cambios estudiados, se busca el que menos cambios tenga respecto a la instancia original. En \cite{Delaney2021} implementan \textit{Native Guide}, el cual se basa en encontrar la instancia más cercana que cambie la salida del modelo (también conocida como \textit{nearest-unlike neighbour}); se usa una distancia Manhattan con búsqueda de tipo \textit{greedy}. En \cite{Filali2022} detallan Timex, donde minimizan tres objetivos simultáneos: diferencia de objetivos, diferencia respecto a la instancia original y, como novedad, la diferencia entre baricentros de las series; luego generan un nuevo conjunto de datos con distintas predicciones que se usa para encontrar el contrafactual mediante distancia Manhattan. En \cite{Bahri2024} presentan DiscoX, en el que usan una medida basada en series de discordia (una serie temporal de discordia es una subsecuencia que tiene la mayor distancia respecto a otra $T$, es decir, una serie de discordia en $T$ con referencia a $T'$ tiene forma similar a $T$ pero muy distinta a $T'$), y construyen un nuevo conjunto de datos que se usa para buscar el contrafactual mediante perturbaciones. En \cite{Lang2023} describen SPARCE, un método de generación que se centra más en generar contrafactuales variados; para ello entrenan una red GAN que luego optimizan por descenso de gradiente. La función de pérdida combina tres términos: uno de validez, uno de proximidad y otro de variedad. En \cite{Qi2024} proponen TIME-CF, que usa \textit{shapelets} (segmentos de la serie con formas características); el algoritmo de generación de contrafactuales trata de sustituir los \textit{shapelets} (generados con una red GAN) hasta hallar el contrafactual. En \cite{Wang2024} exponen Glacier, donde primero entrenan un autoencoder, sobre el que aplican descenso de gradiente minimizando una función de pérdida que combina similitud con plausibilidad; el resultado es un único contrafactual. Continuando con \cite{Xiangyu2025}, implementan CFWoT (\textit{Counterfactuals Without Training Data}), en el que mediante aprendizaje por refuerzo se varía la instancia original hasta que se encuentra un estado en el que se ha alcanzado el objetivo contrafactual; se reportan los contrafactuales de menor a mayor distancia respecto a la instancia original.


Por último, aunque la literatura es escasa, para los casos de regresión (o predicción) en series temporales se encuentran los siguientes métodos: en \cite{Yan2023} presentan CounTS, un modelo de series temporales inherentemente interpretable que aplica directamente el \textit{framework} de Pearl. En CounTS usan como base un modelo de redes de neuronas bayesianas, con el que no solo se entrena el valor de salida sino que se va entrenando para conocer la distribución de probabilidad original de los datos. Con este modelo, es posible hacer tanto el paso de inferencia como el paso de generar el contrafactual. El contrafactual se genera mediante descenso del gradiente hasta que la instancia contrafactual tenga llega a una probabilidad de existencia definida. Por último, en \cite{Wang2023} implementan ForecastCF, en el que dado un modelo de series temporales diferenciable $f(\cdot)$, se busca mediante descenso de gradiente el contrafactual dentro de un intervalo $[\alpha, \beta]$ usando la función de pérdida $\mathcal{L} =\|f(x^*) - \alpha\| + \|\beta - f(x^*)\|$; las principales innovaciones fueron determinar el contrafactual en un rango concreto y aplicar el descenso del gradiente de contrafactuales a un modelo de regresión.

La literatura actual no ha desarrollado métodos de explicación contrafactual de regresión en la misma medida que para el caso de clasificación \cite{Guidotti2022}.

\section{Modelos surrogados}
...
Los modelos surrogados pueden ser tanto globales (si se entrenan sobre la distribución completa) o locales (si se usa un subconjunto) \cite{Kamath2021, molnar2025}. En ambos casos, también se conoce al proceso de obtener modelos surrogados como "destilación", sobre todo en un contexto de modelos de redes de neuronas \cite{hinton2015}. Este trabajo se centrará en los modelos surrogados locales, pues se ajustan al objetivo del problema a resolver. Los modelos surrogados locales tienen un amplio uso en la ingeniería aeroespacial, por ejemplo para calcular las turbulencias en las puntas de las alas o el frenado de un satélite en la reentrada a la atmósfera \cite{Ghosh2021}.

Respecto al uso en inteligencia artificial, la metodología de interpretabilidad con modelos surrogados ha evolucionado de usar un único método para analizar todo el modelo, a generar un \textit{ensemble} de modelos, calibrados para distintas zonas de la distribución de entrada del modelo a interpretar \cite{Pitra2025}; la potencia del modelo surrogado depende en gran medida de la región de estudio y de la no linearidad del modelo original. En particular, se consideran métodos de frontera aquellos que usan procesos gaussianos, sobre todo cuando es necesario dar una cota de error máxima al modelo surrogado. En \cite{Jia2020} presentan AutoSM (\textit{Auto Surrogate Model}), un algoritmo genético que dado el tamaño de muestra, escala, ruido, etc. ajusta los hiperparámetros de de un árbol de decisión, este árbol de decisión luego da lugar a distintos modelos surrogados que se usan finalmente para la interpretración. En \cite{Hess2013} explican el método \textit{PROGRESS} (\textit{Progressive Reinforcement-Learning-Based Surrogate Selection}), un método de aprendizaje por refuerzo para seleccionar un \textit{ensemble} óptimo a partir de una familia de modelos surrogados. En \cite{Raissi2016DeepMG} y \cite{Kerleguer2024} usan una combinación de procesos gaussianos y redes neuronales para conseguir que el modelo surrogado capture las características no lineales del modelo original, especialmente si existen saltos de discontinuidad. En \cite{Yuan2024} se usan modelos surrogados de tipo árbol bayesiano para poder dar garantías estadísticas sobre la incertidumbre del error; este tipo de modelo surrogados son útiles para que la interpretación pueda tener un error no solo especificado, sino también calibrado. En \cite{Boodaghidizaji2022} presentan un modelo surrogado únicamente basado en redes de neuronas profundas para problemas de física de fluidos, adquiriendo mejor rendimiento que los procesos gaussianos en condiciones de alta dimensionalidad. Respecto a la selección del subconjunto de entrenamiento del modelo surrogado, se suele optar por usar de forma completa la región de interés.

Por último el método canónico de generación de modelos surrogados lo encontramos en LIME. LIME es un método propuesto por \cite{Ribeiro2016} en el que dada una instancia $\mathbf{x}$, se genera una vecindad de puntos (perturbación), y por medio de la ejecución repetida del modelo original se obtiene un nuevo subconjunto de datos, ignorando los datos originales. El modelo surrogado es inherentemente interpretrable, y se suelen usar modelos surrogados basados en MCO, lasso o árboles de decisión. El modelo surrogado funciona como una aproximación local fidedigna. Sin pérdida de generalidad, se puede describir el caso de LIME con MCO, (que es el de mayor interés en este trabajo):
\begin{enumerate}
    \item Se escoge un punto de interés (pivote) sobre el que centrar el modelo surrogado.
    \item Genera una perturbación alrededor del punto de interés usando una distribución de media y varianza igual por cada columna del modelo.
    \item Las muestras generadas en la perturbación se ponderan de acuerdo a la cercanía con la instancia original. Por defecto LIME usa (en datos tabulares) un kernel de suavizado exponencial.
    \item Con la muestra relevante, se entrena el modelo surrogado. Por defecto se usa una regresión tipo Ridge \footnote{\url{https://github.com/marcotcr/lime/blob/ce2db6f20f47c3330beb107bb17fd25840ca4606/lime/lime_tabular.py\#L265}} , pero se puede usar cualquier modelo de regresión.
\end{enumerate}
LIME por defecto usa un kernel con suavizado exponencial para escoger la vecindad de puntos. El tamaño de kernel es 0.75 por la raíz cuadrada del número de columnas del modelo surrogado \footnote{\url{https://github.com/marcotcr/lime/blob/ce2db6f20f47c3330beb107bb17fd25840ca4606/lime/lime_tabular.py\#L195}}. Este valor por defecto afecta completamente al rendimiento y parámetros finales del modelo surrogado, siendo además que el valor del tamaño de kernel y el hecho de usar un kernel de suavizado exponencial, son elecciones arbritarias \cite{molnar2025}.

Una de los problemas no resueltos en la literatura es escoger un modelo surrogado que sepamos que tiene un error acotado, en qué regiones y qué vecindad de datos se debe usar para generar el modelo surrogado \cite{Werth2018}. La tendencia general es en escoger modelos surrogados que tengan un rendimiento adecuado a lo largo de varias métricas, como varianza, media o interpretabilidad, y no solo escoger el modelo surrogado con el menor error. El paso de escoger un modelo surrogado (y las regiones aplicables) cada vez se apoya más en métodos automáticos, en vez de ser un paso previo a la interpretación del modelo; el hecho de que existan regiones más explicables de forma automática da una pista al investigador de dónde el modelo original se comporta de forma esperada y dónde no.

\chapter{Análisis del problema}
\label{cap:analisis}

El problema que aborda \texttt{CFGTS} puede formularse del siguiente modo: dado un modelo autorregresivo multi-salida ya ajustado $\hat{f}: \mathbb{R}^{n_{\text{In}}} \rightarrow \mathbb{R}^{D}$, una única instancia original $\mathbf{x}_0$ y un objetivo contrafactual $\mathbf{y}^{*} \in \mathbb{R}^{D}$, se desea encontrar las perturbaciones de $\mathbf{x}_0$ que satisfagan simultáneamente cuatro requisitos:
...
El algoritmo \texttt{CFGTS} se ejecuta en tres fases secuenciales:
\begin{itemize}
    \item Generación de candidatos contrafactuales mediante optimización multi-objetivo.
    \item Generación de un conjunto de cobertura para enriquecer el espacio muestral.
    \item Evaluación causal de cada contrafactual generado.
\end{itemize}
...
Identificando la instancia genérica $\mathbf{x}$ con la instancia concreta $\mathbf{x}_0$, el problema contrafactual se resuelve mediante optimización bi-objetivo sobre $J_1$ (ecuación~\eqref{eq:formal_j1}) y $J_2$ (ecuación~\eqref{eq:formal_j2}) mediante un algoritmo de tipo \textit{Tree-structured Parzen Estimator} (TPE), obteniendo un frente de Pareto:
$$\mathcal{P} = \{\mathbf{x}' \in \mathcal{F}(r_{\min}, r_{\max}) : \mathbf{x}' \text{ es Pareto-óptimo respecto de } J_1 \text{ y } J_2\}$$
Las soluciones del frente de Pareto $\mathcal{P}$ constituyen el conjunto inicial de candidatos contrafactuales $\mathcal{C}_0$, según el Algoritmo~\ref{alg:counterfactual}.
...
\begin{algorithm}[H]
    ...
    Calcular $J_1(\mathbf{x}')$ según (\ref{eq:formal_j1})\;
    Calcular $J_2(\mathbf{x}')$ según (\ref{eq:formal_j2})\;
    Registrar $(\mathbf{x}', J_1, J_2)$ en el estudio\;
...
\begin{definition}[Objetivo medio de cobertura]
    El objetivo global minimizado por \texttt{CFGTS} es la media de las discrepancias de cobertura a través de las $D$ salidas:
...
Instanciando el intervalo de cobertura de la ecuación~\eqref{eq:formal_coverage_interval} con $\hat{\mathbf{y}} = \hat{\mathbf{y}}_0 = \hat{f}(\mathbf{x}_0)$ y $\hat{\mathbf{y}}' = \mathbf{y}^{*}$, cada componente $d$ tiene diámetro $3|\hat{y}_{0,d} - y^{*}_d|$ (Teorema~\ref{thm:cov_diameter}). Dada una muestra de predicciones por componente $\{z_{1,d}, \ldots, z_{N,d}\} \subset \mathbb{R}$, se filtran los puntos dentro del intervalo, se normalizan mediante $\varphi_{a_d,b_d}$ (ecuación~\eqref{eq:formal_rescale}) y se evalúa la discrepancia estelar centrada $L^2$ (ecuación~\eqref{eq:formal_discrepancy}, con los núcleos~\eqref{eq:formal_disc_kernel} y~\eqref{eq:formal_pair_kernel}). Si ninguna predicción cae dentro del intervalo de una componente, se asigna una penalización grande a esa componente. La generación del conjunto de cobertura se formula entonces como la minimización del objetivo medio de cobertura $\operatorname{MeanCov}(\mathcal{S})$, resuelta igualmente mediante TPE, según el Algoritmo~\ref{alg:coverage}.
...
\begin{algorithm}[H]
    ...
    Calcular $\operatorname{MeanCov}(\mathcal{S})$ según (\ref{eq:formal_discrepancy})\;
...
El conjunto de vértices del grafo \texttt{CFGTS} está formado por $n_{\text{In}} + D$ nodos: $n_{\text{In}}$ variables de entrada $X_0, \ldots, X_{n_{\text{In}}-1}$ y $D$ variables de salida $Y_0, \ldots, Y_{D-1}$.
...
Las aristas del grafo causal \texttt{CFGTS} se construyen según dos reglas:
\begin{enumerate}
...
\subsubsection{CausalModel vs Naive LIME}
...
Fijada una salida $d$, la comparación correcta se formula respecto del \textbf{efecto total} $\boldsymbol{\tau}^{(d)}$, no respecto del coeficiente directo condicionado, y se consideran dos modelos alternativos:
\begin{itemize}
    ...
    \item \textbf{Naive LIME} (MCO ajustado, pero sin usar el DAG):
          $$\widehat{\boldsymbol{\beta}}_{\text{mco}}^{(d)} = \boldsymbol{\tau}^{(d)} + \mathbf{b}^{(d)},$$
          donde $\mathbf{b}^{(d)} \in \mathbb{R}^{n_{\text{In}}}$ es el vector de sesgo (también conocido como término independiente).
\end{itemize}

\subsubsection{Algoritmo}

A partir del conjunto de datos $\mathcal{D} = \mathcal{C}_0 \cup \mathcal{S}$ se construye el grafo causal $\mathcal{G} = (V, E)$ según las reglas de construcción anteriores y, sobre él, se estima $\widehat{\boldsymbol{\beta}}^{(d)}$ mediante DAG con el criterio de puerta trasera, para cada salida $d$ y cada variable de entrada $X_j$. A continuación, para cada contrafactual $\mathbf{x}'_k \in \mathcal{C}_0$ y cada salida $d$ se calculan el cambio esperado y el cambio real (ecuaciones~\eqref{eq:formal_expected_delta} y~\eqref{eq:formal_actual_delta}) y la puntuación de consistencia causal $s_{k,d}$ (ecuación~\eqref{eq:formal_causal_score}), agregada finalmente en la puntuación media $s_k$. El Algoritmo~\ref{alg:scoring} resume este procedimiento; el Teorema~\ref{thm:dag_dominates} y sus generalizaciones multi-salida justifican por qué estimar $\widehat{\boldsymbol{\beta}}^{(d)}$ mediante el criterio de puerta trasera sobre $\mathcal{G}$, en lugar de una regresión lineal ingenua sobre todos los retardos, produce puntuaciones al menos tan altas.
...
\begin{algorithm}[H]
    ...
    \For{$d \leftarrow 1$ \KwTo $D$}{
        \ForEach{variable $X_j$, $j = 1, \ldots, n_{\text{In}}$}{
            Estimar $\widehat{\beta}_j^{(d)}$ por regresión lineal con criterio de puerta trasera sobre $\mathcal{G}$\;
        }
    }
    \ForEach{$\mathbf{x}'_k \in \mathcal{C}_0$}{
        \For{$d \leftarrow 1$ \KwTo $D$}{
            $\Delta y^{\text{esperado}}_{k,d} \leftarrow \sum_{j=1}^{n_{\text{In}}} \widehat{\beta}_j^{(d)} \cdot (x'_{k,j} - x_{0,j})$\;
...
\textbf{Paso 1. Grafo causal desplegado en el tiempo.} Un sistema dinámico lineal como el de la ecuación~\eqref{eq:formal_varp} puede ``desplegarse'' en un grafo dirigido acíclico cuyos nodos son pares (variable, instante) y cuyas aristas son exactamente los coeficientes de retardo. La entrada $(u, v)$ de la matriz $A_k$ es el peso de la arista que va del nodo $(v,\, t-k)$ al nodo $(u,\, t)$. Cada matriz de retardo $A_k$ describe, por tanto, las aristas que saltan exactamente $k$ instantes hacia el futuro. La Figura~\ref{fig:dag_desplegado} muestra este despliegue para un sistema con dos variables, $X$ e $Y$, y tres retardos.

\begin{figure}[H]
    \ffigbox[\FBwidth]{
        \caption[Grafo causal para serie temporal bivariante con dos retardos]{Grafo causal temporal para un sistema con dos variables, $X$ e $Y$ ---es decir, $\mathbf{Z}_t = (X_t,\, Y_t)^\top$---, y dos retardos ($n_{\text{In}} = 4$, $D = 2$). Cada nodo es un par (variable, instante) y los nodos de salida $X_t$ e $Y_t$ aparecen resaltados en azul. \textbf{Aristas azules continuas (regla~a):} toda variable de entrada apunta a cada salida. \textbf{Aristas grises discontinuas (regla~b):} toda variable de retardo mayor apunta a toda variable de retardo menor, respetando la causalidad temporal; son las 4 aristas temporales entre columnas de entrada ($X_{t-2} \to X_{t-1}$, $X_{t-2} \to Y_{t-1}$, $Y_{t-2} \to X_{t-1}$, $Y_{t-2} \to Y_{t-1}$) que materializan las trayectorias mediadas y permiten al criterio de \textit{backdoor} identificar el efecto total sin bloquear dichas trayectorias. El grafo contiene en total $8 + 4 = 12$ aristas dirigidas y no presenta ciclos.}
        \label{fig:dag_desplegado}
    }{
        \begin{tikzpicture}[
                x=55mm, y=18mm,
                nodo/.style={circle, draw=azulUC3M, very thick, minimum size=13mm, inner sep=0pt, font=\small},
                salida/.style={circle, draw=azulUC3M, line width=1.4pt, fill=azulUC3M!15, minimum size=13mm, inner sep=0pt, font=\small},
                ra/.style={-{Stealth[length=2.2mm]}, thick, azulUC3M},
                rb/.style={-{Stealth[length=2.2mm]}, semithick, gray45, densely dashed}
            ]
            % Nodes: two lags × two variables + two outputs
            \node[nodo]   (x2) at (0, 1)  {$X_{t-2}$};
            \node[nodo]   (y2) at (0,-1)  {$Y_{t-2}$};
            \node[nodo]   (x1) at (1, 1)  {$X_{t-1}$};
            \node[nodo]   (y1) at (1,-1)  {$Y_{t-1}$};
            \node[salida] (x0) at (2, 1)  {$X_{t}$};
            \node[salida] (y0) at (2,-1)  {$Y_{t}$};

            % Rule (b): higher-lag → lower-lag among inputs (4 edges)
            \draw[rb] (x2) -- node[midway, above, font=\scriptsize] {$[A_1]_{XX}$} (x1);
            \draw[rb] (y2) -- node[midway, below, font=\scriptsize] {$[A_1]_{YY}$} (y1);
            \draw[rb] (x2) to[bend right=22] node[pos=0.38, right, font=\scriptsize] {$[A_1]_{YX}$} (y1);
            \draw[rb] (y2) to[bend left=22]  node[pos=0.38, left,  font=\scriptsize] {$[A_1]_{XY}$} (x1);

            % Rule (a): every input → X_t (4 edges)
            \draw[ra] (x1) -- node[midway, above, font=\scriptsize] {$[A_1]_{XX}$} (x0);
            \draw[ra] (y1) to[bend right=22] node[pos=0.55, right, font=\scriptsize] {$[A_1]_{XY}$} (x0);
            \draw[ra] (x2) to[bend left=30]  node[pos=0.45, above, font=\scriptsize] {$[A_2]_{XX}$} (x0);
            \draw[ra] (y2) to[bend right=18] node[pos=0.62, right, font=\scriptsize] {$[A_2]_{XY}$} (x0);

            % Rule (a): every input → Y_t (4 edges)
            \draw[ra] (y1) -- node[midway, below, font=\scriptsize] {$[A_1]_{YY}$} (y0);
            \draw[ra] (x1) to[bend left=22]  node[pos=0.55, left,  font=\scriptsize] {$[A_1]_{YX}$} (y0);
            \draw[ra] (y2) to[bend right=30] node[pos=0.45, below, font=\scriptsize] {$[A_2]_{YY}$} (y0);
            \draw[ra] (x2) to[bend left=18]  node[pos=0.62, left,  font=\scriptsize] {$[A_2]_{YX}$} (y0);
        \end{tikzpicture}
    }
\end{figure}

\textbf{Paso 2. Efecto directo: una sola arista.} El efecto directo a horizonte $h+1$ es la contribución de la única arista de retardo $h+1$, sin pasos intermedios: $\mathbf{D}^{(h+1)} = A_{h+1}$. Gráficamente es la flecha que conecta $(v,\, t-(h+1))$ con $(u,\, t)$ de un solo salto.

\textbf{Paso 3. Efecto total: suma sobre todas las trayectorias.} En un grafo lineal, el efecto que se transmite a lo largo de una trayectoria dirigida es el \emph{producto} de los pesos de sus aristas (véase \ref{fig:dag_mediacion_efectos}), y el efecto total entre dos nodos es la \emph{suma} sobre todas las trayectorias dirigidas que los conectan. El caso base $\mathbf{T}^{(0)} = I_n$ codifica que, a horizonte cero, cada variable solo se afecta a sí misma con peso~$1$. La recursión de la ecuación~\eqref{eq:formal_total_lag_effect} clasifica cada trayectoria por su \emph{última} arista: si esa última arista tiene retardo $j+1$ (coeficiente $A_{j+1}$, aplicado por la izquierda por ser el último paso antes de llegar a $t$), el tramo restante hasta el instante intermedio acumula todas las maneras de recorrer el horizonte $h-j$, que es justamente $\mathbf{T}^{(h-j)}$.

\textbf{Paso 4. Efecto mediado y descomposición.} El efecto mediado $\mathbf{M}^{(h+1)}$ es la misma suma del efecto total pero \emph{sin} su último término (el directo): recoge todas las trayectorias que pasan por al menos un nodo intermedio. Separando el término directo del resto de la suma se obtiene la identidad
$$\mathbf{T}^{(h+1)} = \underbrace{\mathbf{D}^{(h+1)}}_{\text{trayectorias directas}} + \underbrace{\mathbf{M}^{(h+1)}}_{\text{trayectorias mediadas}}$$
La Figura~\ref{fig:efecto_total_h2} ilustra el caso de dos retardos, donde la única ruta mediada $\mathbf{Z}_{t-2} \to \mathbf{Z}_{t-1} \to \mathbf{Z}_t$ aporta $A_1^2$, de modo que $\mathbf{T}^{(2)} = A_2 + A_1^2$.

\begin{figure}[H]
    \ffigbox[\FBwidth]{
        \caption[Descomposición del efecto total a dos retardos]{Las dos rutas dirigidas de $\mathbf{Z}_{t-2}$ a $\mathbf{Z}_t$ en un sistema VAR(2): la arista directa de retardo~$2$ (efecto directo $\mathbf{D}^{(2)} = A_2$, en azul) y la ruta mediada que atraviesa el instante intermedio $t-1$ (efecto mediado $\mathbf{M}^{(2)} = A_1^2$, discontinua). El efecto total es su suma: $\mathbf{T}^{(2)} = A_2 + A_1^2$.}
        \label{fig:efecto_total_h2}
    }{
        \begin{tikzpicture}[
                node distance=30mm,
                nodo/.style={circle, draw=azulUC3M, very thick, minimum size=13mm, inner sep=1pt, font=\small},
                directa/.style={-{Stealth[length=2.6mm]}, very thick, azulUC3M},
                mediada/.style={-{Stealth[length=2.6mm]}, thick, gray45, densely dashed}
            ]
            \node[nodo] (z2) {$\mathbf{Z}_{t-2}$};
            \node[nodo, right=of z2] (z1) {$\mathbf{Z}_{t-1}$};
            \node[nodo, right=of z1] (z0) {$\mathbf{Z}_{t}$};

            \draw[mediada] (z2) -- node[midway, above] {$A_1$} (z1);
            \draw[mediada] (z1) -- node[midway, above] {$A_1$} (z0);
            \draw[directa] (z2) to[bend right=42] node[midway, below] {$A_2$} (z0);
        \end{tikzpicture}
    }
\end{figure}

\textbf{Paso 5. Por qué MCO y el grafo causal divergen.} El estimador causal apunta al \emph{efecto total} $\boldsymbol{\tau}^{(d)}$, por lo que ajusta solo por confusores y \emph{no} condiciona sobre los retardos intermedios que se encuentran \emph{sobre} las trayectorias causales: conserva las contribuciones mediadas. La regresión MCO, en cambio, regresa la salida sobre \emph{todos} los retardos simultáneamente; el coeficiente de cada retardo se interpreta entonces manteniendo fijos los demás, incluidos los retardos intermedios. Pero fijar un mediador \emph{bloquea} la trayectoria que pasa por él (la misma lógica del operador $\mathrm{do}(\cdot)$ de la subsección sobre mediación), de modo que el MCO recupera únicamente la arista directa $\mathbf{D}^{(h+1)} = A_{h+1}$. La Figura~\ref{fig:mco_bloqueo} muestra este bloqueo.

\begin{figure}[H]
    \ffigbox[\FBwidth]{
        \caption[Bloqueo de la mediación por el MCO ajustado]{Al ajustar por todos los retardos, el MCO condiciona sobre el mediador $\mathbf{Z}_{t-1}$ (nodo sombreado). Condicionar sobre el mediador \emph{bloquea} la ruta mediada (aristas punteadas), de modo que el MCO solo recupera la arista directa $A_2$. Estima así el efecto directo $\mathbf{D}^{(2)} = A_2$ en lugar del efecto total, con un sesgo $\mathbf{B}_{\mathrm{MCO}}^{(2)} = -\mathbf{M}^{(2)} = -A_1^2$.}
        \label{fig:mco_bloqueo}
    }{
        \begin{tikzpicture}[
                node distance=30mm,
                nodo/.style={circle, draw=azulUC3M, very thick, minimum size=13mm, inner sep=1pt, font=\small},
                cond/.style={circle, draw=azulUC3M, very thick, fill=gray75, minimum size=13mm, inner sep=1pt, font=\small},
                directa/.style={-{Stealth[length=2.6mm]}, very thick, azulUC3M},
                bloqueada/.style={-{Stealth[length=2.6mm]}, thick, gray75, densely dotted}
            ]
            \node[nodo] (z2) {$\mathbf{Z}_{t-2}$};
            \node[cond, right=of z2] (z1) {$\mathbf{Z}_{t-1}$};
            \node[nodo, right=of z1] (z0) {$\mathbf{Z}_{t}$};

            \draw[bloqueada] (z2) -- node[midway, above] {$A_1$} (z1);
            \draw[bloqueada] (z1) -- node[midway, above] {$A_1$} (z0);
            \draw[directa] (z2) to[bend right=42] node[midway, below] {$A_2$} (z0);
        \end{tikzpicture}
    }
\end{figure}

\textbf{Paso 6. El sesgo es exactamente la parte mediada.} La descomposición del Paso~4 resulta en
$$\mathbf{B}_{\mathrm{MCO}}^{(h+1)} = \mathbf{D}^{(h+1)} - \mathbf{T}^{(h+1)} = \mathbf{D}^{(h+1)} - \bigl(\mathbf{D}^{(h+1)} + \mathbf{M}^{(h+1)}\bigr) = -\mathbf{M}^{(h+1)}$$
El sesgo al estimar por MCO es la contribución mediada omitida con signo negativo. En el caso de dos retardos, el MCO estima $A_2$ mientras que el efecto total es $A_2 + A_1^2$.
...
\begin{theorem}[Sesgo del MCO ajustado]
    Para todo horizonte $h+1 \geq 1$:
...
\begin{corollary}[Caso VAR(2)]
    ...
    mientras que el sesgo del MCO ajustado sobre el retardo $2$ viene dado por
...
\paragraph{CausalModel vs Naive LIME por objetivo}
...
\paragraph{Alcance más allá del caso lineal}

Los teoremas de dominancia anteriores se enuncian bajo la hipótesis de que cada componente $f_d$ es lineal. Conviene precisar qué ocurre cuando el modelo ajustado $\hat{f}$ es arbitrario (un \textit{Random Forest}, una red neuronal, etc.).

En primer lugar, las propiedades que garantizan que la puntuación está \textbf{bien definida} ($s_d \in (0, 1]$, que vale $1$ en la instancia original y que el cambio esperado es lineal en la perturbación) se demuestran para $\hat{f}$ \emph{arbitrario}, sin hipótesis de linealidad. El algoritmo, por tanto, opera correctamente con cualquier modelo.

En segundo lugar, la \emph{dirección} de la ventaja del modelo causal sobre la regresión MCO tampoco depende de la linealidad: el argumento es que condicionar sobre retardos intermedios \textbf{bloquea las trayectorias mediadas}, de modo que un ajuste MCO estima un efecto directo controlado en lugar del efecto total. Este es un resultado causal de identificación, válido sea cual sea la forma funcional del sistema.

La no linealidad impide una \emph{desigualdad estricta universal punto a punto}, por dos motivos: (i) el efecto total deja de tener forma cerrada matricial: no existe un coeficiente $A_\tau$ constante, sino una cantidad que depende del punto $\mathbf{x}$ y de la dirección de la perturbación; y (ii) la puntuación compara contra el cambio real $\Delta y_d^{\text{real}} = f_d(\mathbf{x}') - f_d(\mathbf{x})$, que es no lineal, por lo que para perturbaciones grandes, ninguna estimación lineal de coeficientes lo reproduce exactamente y, en casos patológicos, el vector sesgado del MCO podría quedar circunstancialmente más cerca del cambio real que el efecto total.
...
La implementación de \texttt{CFGTS} se rige por los siguientes principios:
...
\begin{enumerate}
    ...
    \item \textbf{Modelo pre-ajustado de scikit-learn.} El parámetro principal es un modelo de regresión que implemente la interfaz \texttt{RegressorMixin} de scikit-learn. El modelo \textbf{debe estar previamente ajustado} (\textit{fitted}): durante la instanciación, \texttt{CFGTS} invoca \texttt{check\_is\_fitted()} de scikit-learn y lanza una excepción si el modelo no ha sido entrenado. Esta decisión permite que \texttt{CFGTS} sea agnóstico al tipo de regresor utilizado (regresión lineal, \textit{Random Forest}, redes neuronales envueltas en scikit-learn, etc.), siempre que exponga el método \texttt{predict()}. Cuando el problema tiene varias salidas, el modelo puede ser nativamente multi-salida o estar envuelto, por ejemplo, en \texttt{MultiOutputRegressor}.
...
\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Dependencias principales de \texttt{CFGTS} y su función}\label{tab:dependencies}}
...
\begin{table}[H]
    ...
    {\begin{tabular}{|p{4cm}|p{3.2cm}|p{1.5cm}|p{4.5cm}|}
            \hline
            ...
            \texttt{counterfactual
                \_value}  & \texttt{DataFrame}        & ---              & Vector objetivo $\mathbf{y}^{*}$                                                                                                                                                                             \\
            \hline
            ...
            \hline
            \texttt{n\_trials}      & \texttt{int | None}       & \texttt{None}    & Número máximo de ensayos por fase. Cada fase se detiene al alcanzar el primero de los dos límites, \texttt{n\_trials} o \texttt{timeout}; fijarlo hace los resultados independientes de la máquina \\
            \hline
            \texttt{n\_coverage
                \_trials}     & \texttt{int | None}       & \texttt{None}    & Sustituye a \texttt{n\_trials} solo en la Fase~2. Si es \texttt{None}, se emplea \texttt{n\_trials}                                                                                                      \\
            \hline
...
\texttt{CFGTS} requiere Python~3.11 o superior. La instalación se realiza mediante \texttt{pip}:
...
El Código~\ref{lst:usage} muestra un ejemplo completo de uso de \texttt{CFGTS} con un modelo multi-salida y variables temporales con retardos. La convención de nombres \texttt{X\_t}, \texttt{X\_t-1}, \texttt{Y\_t-1}, etc., es utilizada por el algoritmo para inferir automáticamente las aristas temporales del grafo causal.

\begin{lstlisting}[language=Python, caption={Ejemplo de uso de \texttt{CFGTS} con series temporales}, label={lst:usage}]
...
\begin{itemize}
    \item El modelo se entrena con datos tabulares donde las entradas son variables temporales retardadas y las salidas corresponden al instante actual. \texttt{CFGTS} no impone restricciones sobre el tipo de regresor: cualquier estimador de scikit-learn que implemente \texttt{predict()} y produzca una salida de dimensión $D$ es compatible.
...
En este capítulo se presenta una evaluación contra la implementación del algoritmo \texttt{CFGTS}, descrito en el Capítulo~\ref{cap:propuesta}. El protocolo de evaluación está basado en el trabajo de \cite{Wang2023} y \cite{Kan2024}. Para ello se intenta responder a la pregunta, \textit{¿incorporar la puntuación causal aporta información relevante al contrafactual?}

\section{Protocolo experimental}

\subsection{Conjuntos de datos}

Se emplean tres conjuntos de datos, resumidos en la Tabla~\ref{tab:res_datos}. El primero es un conjunto sintético de 20 series AR(2) estacionarias de longitud variable y mecanismo generador lineal, el segundo es un conjunto sintético bivariante VAR(4) de dos variables, y el tercero es la serie de actividad solar \textit{Sunspots} (1700-2008), no lineal y con un ciclo conocido de (aproximadamente) once años.

Cada serie se entrena con los métodos usuales para series temporales, con un split 60, 20, 20, y se normaliza cada variable por \textit{min-max}.

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Conjuntos de datos empleados en la evaluación}\label{tab:res_datos}}
    {\begin{tabular}{|p{3.4cm}|c|c|c|c|}
            \hline
            \textbf{Conjunto} & \textbf{Vars.} & \textbf{Series} & \textbf{Vent. test} & \textbf{Instancias} \\
            \hline
            Sintético AR(2)   & 1              & 20              & 46                  & 15                  \\
            \hline
            Sintético VAR(4)  & 2              & 20              & 66                  & 15                  \\
            \hline
            \textit{Sunspots} & 1              & 1               & 6                   & 6                   \\
            \hline
        \end{tabular}}
\end{table}

\subsection{Modelos y objetivos}

A cada conjunto de datos se ajustan dos modelos: una regresión lineal y un \textit{Random Forest}. El valor objetivo de cada instancia se define como una ventana de cambio porcentual ($cp$), $y^{*}_d = \operatorname{mediana}(\mathbf{x}_{0,d}) \cdot (1 + cp)$, con $cp = 0{,}50$.

La Tabla~\ref{tab:res_protocolo} recoge la configuración, que constituye 216 ejecuciones de CFGTS.

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Configuración experimental de la evaluación de CFGTS}\label{tab:res_protocolo}}
    {\begin{tabular}{|p{6.2cm}|p{5.8cm}|}
            \hline
            ...
            Partición                                             & 60/20/20                                                        \\
            \hline
            Normalización                                         & \textit{min-max}                                                \\
            \hline
            Paso entre ventanas de prueba                         & $p + T = 9$                                                     \\
            \hline
            Modelos                                               & Regresión lineal y \textit{Random Forest}                       \\
            \hline
            \texttt{n\_trials} (Fase~1)                           & 300                                                             \\
            \hline
            \texttt{n\_coverage\_trials} (Fase~2)                 & 10                                                              \\
            \hline
            \texttt{timeout} (tope de seguridad)                  & 60\,s                                                           \\
            \hline
            $[r_{\min}, r_{\max}]$                                & $[0,\ 3]$                                                       \\
            \hline
            \texttt{n\_walk\_steps}                               & 1                                                               \\
            \hline
            ...
            \hline
            Semilla base                                          & 42\footnote{El sentido de la vida, el universo y todo lo demás} \\
            \hline
        \end{tabular}}
\end{table}

\subsection{Magnitudes registradas}

De cada ejecución se analizan los contrafactuales generados, y sobre ellos se calcula:

\begin{itemize}
    \item $\bar{s}$, la puntuación media, el estadístico principal.
    \item $s_{K}$, el máximo sobre una submuestra aleatorio de $K = 5$ contrafactuales. Al fijar $K$ podemos comparar ejecuciones de \texttt{CFGTS} que generan distinta cantidad de contrafactuales.
    \item $\bar{s}_{\text{ctrl}}$, la puntuación media del control aleatorio: 25 perturbaciones de dirección aleatoria que se puntúan invocando \texttt{score\_candidates()} sobre el mismo objeto \texttt{CFGTS}. Comparten, por tanto, los mismos efectos causales estimados.
    \item $J_{\text{obj}}^{\min}$, el menor error relativo alcanzado en el frente, que mide si el objetivo llegó realmente a satisfacerse.
    \item El número de candidatos por instancia y la desviación típica entre semillas, $\sigma_{\text{sem}}$.
\end{itemize}

\subsection{Validez de los modelos entrenados}

Antes de interpretar el \textit{score} se comprueba que los modelos usados son descripciones razonables de las series. La Tabla~\ref{tab:res_smape} recoge el sMAPE de la predicción recursiva a seis pasos sobre el conjunto de prueba. Ningún modelo es degenerado, de modo que los contrafactuales se generan sobre modelos "realistas".

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption[Error de predicción de los modelos]{sMAPE (\%) de predicción recursivo a seis pasos sobre el conjunto de prueba.}\label{tab:res_smape}}
    {\begin{tabular}{|p{3.6cm}|c|c|}
            \hline
            \textbf{Conjunto} & \textbf{Lineal} & \textbf{Random Forest} \\
            \hline
            Sintético AR(2)   & 7{,}62          & 8{,}07                 \\
            \hline
            Sintético VAR(4)  & 3{,}80          & 3{,}70                 \\
            \hline
            \textit{Sunspots} & 15{,}63         & 12{,}94                \\
            \hline
        \end{tabular}}
\end{table}
...
La Tabla~\ref{tab:res_principal} contiene el resultado central de la evaluación y la Tabla~\ref{tab:res_secundaria} los estadísticos complementarios.

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption[Puntuación causal frente al control aleatorio]{Puntuación media del frente de Pareto ($\bar{s}$, con intervalo de confianza al 95\,\%), puntuación media del control aleatorio ($\bar{s}_{\text{ctrl}}$) y diferencia entre ambas ($\Delta$), con el valor $p$ de la prueba $t$ emparejada. Las medias se toman sobre ventanas de prueba independientes, promediando previamente las tres semillas de cada instancia.}\label{tab:res_principal}}
    {\begin{tabular}{|p{2.9cm}|p{2.4cm}|c|c|c|c|}
            \hline
            \textbf{Conjunto} & \textbf{Modelo} & $\bar{s}$            & $\bar{s}_{\text{ctrl}}$ & $\Delta$   & $p$                 \\
            \hline
            Sintético AR(2)   & Lineal          & ${,}975 \pm 0{,}008$ & ${,}891$                & $+0{,}084$ & $\mathbf{<0{,}001}$ \\
            \hline
            Sintético AR(2)   & Random Forest   & ${,}599 \pm 0{,}044$ & ${,}516$                & $+0{,}082$ & $\mathbf{{,}017}$   \\
            \hline
            Sintético VAR(4)  & Lineal          & ${,}940 \pm 0{,}008$ & ${,}869$                & $+0{,}071$ & $\mathbf{<0{,}001}$ \\
            \hline
            Sintético VAR(4)  & Random Forest   & ${,}537 \pm 0{,}061$ & ${,}413$                & $+0{,}124$ & $\mathbf{<0{,}001}$ \\
            \hline
            \textit{Sunspots} & Lineal          & ${,}948 \pm 0{,}063$ & ${,}917$                & $+0{,}030$ & ${,}26$             \\
            \hline
            \textit{Sunspots} & Random Forest   & ${,}500 \pm 0{,}099$ & ${,}421$                & $+0{,}079$ & ${,}06$             \\
            \hline
        \end{tabular}}
\end{table}
...
\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption[Estadísticos complementarios de la evaluación]{Máximo sobre un submuestreo de $K = 5$ candidatos ($s_K$), menor error relativo alcanzado en el frente ($J_{\text{obj}}^{\min}$), tamaño medio del frente y desviación típica entre semillas.}\label{tab:res_secundaria}}
    {\begin{tabular}{|p{2.9cm}|p{2.4cm}|c|c|c|c|}
            \hline
            \textbf{Conjunto} & \textbf{Modelo} & $s_K$    & $J_{\text{obj}}^{\min}$ & \textbf{Cand.} & $\sigma_{\text{sem}}$ \\
            \hline
            Sintético AR(2)   & Lineal          & ${,}995$ & ${,}001$                & 15{,}8         & ${,}018$              \\
            \hline
            Sintético AR(2)   & Random Forest   & ${,}783$ & ${,}111$                & 11{,}3         & ${,}074$              \\
            \hline
            Sintético VAR(4)  & Lineal          & ${,}983$ & ${,}043$                & 12{,}7         & ${,}029$              \\
            \hline
            Sintético VAR(4)  & Random Forest   & ${,}718$ & ${,}358$                & 8{,}8          & ${,}075$              \\
            \hline
            \textit{Sunspots} & Lineal          & ${,}990$ & ${,}001$                & 12{,}3         & ${,}047$              \\
            \hline
            \textit{Sunspots} & Random Forest   & ${,}786$ & ${,}135$                & 13{,}3         & ${,}072$              \\
            \hline
        \end{tabular}}
\end{table}

El primer patrón es que \textbf{la naturaleza del modelo domina el valor de la puntuación causal}, muy por encima del conjunto de datos. La Figura~\ref{fig:res_scores} muestra el mismo patrón en forma de distribución.

El segundo patrón, y el más importante, es que \textbf{el control aleatorio obtiene puntuaciones inferiores a las de \texttt{CFGTS}}. Este resultado se analiza en la Sección~\ref{sec:res_control}.

\begin{figure}[H]
    \ffigbox[\FBwidth]{
        \caption[Distribución de las puntuaciones de consistencia causal]{Distribución de la puntuación media del frente de Pareto por ejecución, desagregada por conjunto de datos y modelo. Las puntuaciones del modelo lineal (azul) se concentran junto a la unidad en los tres conjuntos, mientras que las del \textit{Random Forest} (naranja) se reparten en torno a ${,}5$ con gran dispersión.}
        \label{fig:res_scores}
    }{
        \includegraphics[width=\linewidth]{fig_score_distribution.png}
    }
\end{figure}

\section{Conjunto de datos sintéticos}

Los conjuntos de datos sintéticos que se han usado permite responder a una de las premisas básicas de este trabajo, un mecanismo generador y modelo lineales inducen una estructura de tipo grafo causal que \texttt{CFGTS} puede capturar.
La experimentación con los conjuntos datos confirman la hipótesis en ambos casos. Con modelo lineal, $\bar{s}$ alcanza ${,}975 \pm 0{,}008$ en AR(2) y ${,}940 \pm 0{,}008$ en VAR(4).

Al sustituir el modelo por un \textit{Random Forest} la puntuación cae a ${,}599 \pm 0{,}044$ en AR(2) y a ${,}537 \pm 0{,}061$ en VAR(4). Aquí la puntuación deja de ser alta pero el control también desciende en paralelo, hasta ${,}516$ y ${,}413$ respectivamente.

La Figura~\ref{fig:res_ejemplo} muestra cualitativamente una ejecución sobre el conjunto AR(2). La predicción se desplaza de forma coherente hacia delante, pese a que la optimización solo controla explícitamente la entrada.

\begin{figure}[H]
    \ffigbox[\FBwidth]{
        \caption[Ejemplo cualitativo de contrafactual generado por CFGTS]{Contrafactual generado por \texttt{CFGTS} sobre una instancia del conjunto sintético AR(2) con modelo lineal. En negro, la ventana de retardos original (trazo discontinuo) y su predicción recursiva a seis pasos (trazo continuo); en verde, la ventana contrafactual y la predicción; en naranja, el valor objetivo $y^{*}$.}
        \label{fig:res_ejemplo}
    }{
        \includegraphics[width=0.85\linewidth]{fig_example.png}
    }
\end{figure}

\section{Conjunto de datos reales}

Para evaluar un conjunto de datos reales, se ha escogido la serie \textit{Sunspots}. Esta serie presenta no linealidad, un período de 11 años, amplitud no estacionaria y heterocedasticidad. Además, los modelos usados son desfavorables en esta serie, al ser modelos de orden 3 (inferior al período de 11). La Tabla~\ref{tab:res_smape} refleja los errores sMAPE obtenidos al ajustar los modelos lineal y Random Forest a los datos.

El comportamiento de la puntuación causal apenas se distingue del observado en el conjunto sintético: ${,}948 \pm 0{,}063$ con modelo lineal y ${,}500 \pm 0{,}099$ con \textit{Random Forest}. La degradación respecto del escenario controlado es de apenas ${,}027$ puntos para el modelo lineal.

Esta insensibilidad es en sí misma el resultado: se recuerda de la Fase~3 de la propuesta que la puntuación causal no mide la fidelidad del modelo al proceso real, sino la \textit{coherencia interna} entre el contrafactual y el mecanismo que el modelo ha aprendido. Un modelo lineal de orden 3 es un mal predictor de la actividad solar (al menos peor que el Random Forest). La consecuencia práctica es que una puntuación causal elevada no debe interpretarse como evidencia de que la explicación sea válida respecto del fenómeno, sino respecto del modelo; si el modelo es una mala representación del fenómeno, la explicación será internamente coherente y a la vez extrínsecamente engañosa.

\section{Contraste con el control aleatorio}
\label{sec:res_control}

La Figura~\ref{fig:res_control} muestra la comparación del \textit{score} causal contra contrafactuales aleatorios. Se esperaría que aquellos que se han buscado mediante el algoritmo tuviesen mejor puntuación.

\begin{figure}[H]
    \ffigbox[\FBwidth]{
        \caption[Puntuación causal de CFGTS frente al control aleatorio]{Puntuación media de consistencia causal de los contrafactuales de \texttt{CFGTS} (azul) frente a contrafactuales aleatorios. Las barras de error son intervalos de confianza al 95\,\%.}
        \label{fig:res_control}
    }{
        \includegraphics[width=\linewidth]{fig_control.png}
    }
\end{figure}

\section{Relación con los objetivos de la Fase 1}
...
\begin{figure}[H]
    \ffigbox[\FBwidth]{
        \caption[Relación entre proximidad, error y consistencia causal]{Todos los contrafactuales generados durante la evaluación, situados en el plano de los dos objetivos de optimización en la Fase~1, y coloreados según su puntuación causal.}
        \label{fig:res_tradeoff}
    }{
        \includegraphics[width=0.8\linewidth]{fig_tradeoff.png}
    }
\end{figure}

\chapter{Marco regulador}
\label{cap:regulacion}

El marco regulador recoge el conjunto de leyes, normas y estándares relevantes para \texttt{CFGTS}. A diferencia de una aplicación final, \texttt{CFGTS} no recopila, almacena ni procesa datos personales: recibe un modelo ya ajustado, una instancia y un objetivo contrafactual, y devuelve explicaciones puntuadas. La legislación aplicable depende del contexto en el que un tercero decida desplegar la herramienta, más que de la propia ejecución del algoritmo. A continuación se mencionan la legislación aplicable, los estándares de programación y el régimen de propiedad intelectual relevantes.

\section{Legislación}

\subsection{Protección de datos: RGPD y LOPDGDD}
El tratamiento de datos personales dentro de la Unión Europea está regulado por el Reglamento (UE) 2016/679, conocido como Reglamento General de Protección de Datos (RGPD) \cite{rgpd2016}, y en España se complementa con la Ley Orgánica 3/2018, de Protección de Datos Personales y garantía de los derechos digitales (LOPDGDD) \cite{lopdgdd2018}. En la evaluación de este trabajo (Capítulo~\ref{cap:resultados}) únicamente se han empleado series sintéticas generadas y la serie \textit{open source} de actividad solar \textit{Sunspots}, por lo que no se ha procesado ningún dato de carácter personal.

Sí existe legislación aplicable si \texttt{CFGTS} se despliega sobre modelos que sí procesan datos personales. Generar y publicar contrafactuales que se acercan a la instancia original mediante pequeñas perturbaciones puede constituir en sí mismo una operación de tratamiento de datos personales (por ejemplo si la instancia contractual es un paciente con sus constantes vitales), y corresponde al operador valorar si resulta necesaria una evaluación de impacto según el artículo 35 del RGPD.

Por otro lado, el artículo 22 del RGPD reconoce el derecho de toda persona a no verse sometida a una decisión basada únicamente en un tratamiento automatizado, así como a obtener una explicación significativa de dicha decisión. En \cite{Wachter2017} se argumenta que una explicación contrafactual es precisamente el mecanismo natural para implementar este derecho sin necesidad de revelar la lógica interna del modelo.

\subsection{Regulación de la inteligencia artificial: AI Act}

El Reglamento (UE) 2024/1689, o Ley de Inteligencia Artificial (AI Act) \cite{aiact2024}, clasifica los sistemas de IA según su nivel de riesgo e obliga a los operadores ciertas obligaciones proporcionales al riesgo. \texttt{CFGTS} no toma decisiones automatizadas y por tanto no constituye un sistema de IA de alto riesgo según el artículo 6 y el Anexo III del reglamento. Pero cuando el modelo explicado sí se emplea en alguno de los ámbitos de alto riesgo el proveedor de ese sistema queda sujeto a las obligaciones de transparencia del artículo 13 y al derecho a explicación de las decisiones individuales del artículo 86. \texttt{CFGTS} puede emplearse como mecanismo para satisfacer estas obligaciones.
...
\begin{enumerate}
    \item Estilo de código: se sigue la guía oficial PEP~8 \cite{pep8}, que define las convenciones de nomenclatura, indentación y organización del código Python. Su cumplimiento se verifica automáticamente mediante el \textit{linter} \texttt{ruff}.
    \item Tipado estático: se aplica PEP~484 (anotaciones de tipos) y PEP~561 (paquetes tipados, marcador \texttt{py.typed}), el código se anota exhaustivamente y se valida con \texttt{mypy}.
    \item Distribución: El fichero \texttt{pyproject.toml} sigue los estándares PEP~517/518 (especificación del sistema de construcción, \texttt{hatchling}) y PEP~621 (metadatos del proyecto en formato declarativo), lo que garantiza una instalación reproducible del paquete con cualquier gestor compatible (como \texttt{pip} o \texttt{uv}).
\end{enumerate}

\section{Propiedad intelectual y licencia}

La formalización e implementación presentados en este trabajo son originales del autor. El paquete \texttt{cfgts} se distribuye bajo licencia Creative Commons Attribution NonCommercial NoDerivatives 4.0 International (CC~BY-NC-ND~4.0), tal y como se recoge en el fichero \texttt{LICENSE} del repositorio y en la portada de este documento. La licencia CC~BY-NC-ND~4.0 solo permite reproducir y redistribuir el paquete sin modificar, exclusivamente con fines no comerciales y reconociendo la autoría original.

La propia Creative Commons desaconseja el uso de sus licencias para software, al no regular la distribución del código fuente frente a la del binario compilado. Una licencia de software reconocida por la \textit{Open Source Initiative} (por ejemplo, MIT o BSD-3-Clause) sería más adecuada para futuras versiones de la implementación si se desea maximizar su adopción.

Por último, las dependencias externas de la Tabla~\ref{tab:dependencies} se distribuyen bajo licencias permisivas de código abierto: MIT (Optuna, DoWhy, Polars), BSD (scikit-learn, SciPy, NumPy, Numba, NetworkX, pandas), la licencia de la \textit{Python Software Foundation} (Matplotlib) y Apache~2.0 (PyArrow). Estas licencias solo exigen atribución y no imponen condiciones sobre la redistribución de obras derivadas, por lo que son compatibles con las restricciones \textit{NonCommercial} y \textit{NoDerivatives} de \texttt{cfgts}. La atribución se hace de forma explícita por el fichero \texttt{pyproject.toml} que se encuentra en la raíz del repositorio.

\chapter{Impacto económico, social, medioambiental y ético}
\label{cap:impacto}
...
El impacto económico se entiende como la posibilidad de alterar el uso de los recursos económico-financieros de una organización por el uso del algoritmo. Principalmente se identifica que el uso de \texttt{CFGTS} tiene el potencial de reducir el coste de incorporar explicabilidad los modelos desplegados:
\begin{enumerate}
    \item El algoritmo no necesita el \textit{dataset} de entrenamiento. Según lo formalizado en Capítulo~\ref{cap:propuesta}, el algoritmo solo necesita un modelo ajustado, una instancia y un objetivo contrafactual. Es relevanto porque con frecuencia las organizaciones necesitan aplicar políticas de retención de datos (es decir, borrado de datos) ya sea por protección de datos o costes de almacenamiento. Auditar un modelo con \texttt{CFGTS} se vuelve una operación de menor coste.
    \item El algoritmo es agnóstico al modelo. Al no imponerse una forma funcional, las organizaciones no tienen que desarrollar algoritmos de explicabilidad específicos por tipo de modelo. La implementación de referencia necesita que el modelo exponga la interfaz de \texttt{RegressorMixin} de scikit-learn, pero una implementación diferente del algoritmo (por ejemplo una basada en \texttt{PyTorch}) puede usar otro tipo de interfaz. Esto reduce la barrera de entrada a la explicabilidad en los equipos de ciencia de datos encargados de los modelos.
\end{enumerate}


\section{Impacto social}

Los modelos de series temporales para los que \texttt{CFGTS} genera contrafactuales pueden usarse para tomar decisiones con un impacto directo sobre la sociedad: demanda eléctrica, parámetros clínicos continuos o número de préstamos en \textit{default} son problemas en los que los modelos de series temporales son usados usualmente. En estos contextos poder generar explicaciones contrafactuales puede mejorar la confianza del operador en las situaciones en las que no es capaz de entender porqué el modelo ha predicho cierta cantidad y no otra. Esta confianza es positiva y entra en línea con las razones de interpretabilidad expuestas en el Capítulo~\ref{cap:estadodelarte}.

Por otra parte, hay que matizar el límite de la confianza que puede aportar el uso del algoritmo a los operadores del modelo, ya que la puntuación de consistencia causal $s$ mide la coherencia del contrafactual contra lo esperado en un proceso causal. Del Capítulo~\ref{cap:resultados} se puede recordar que los modelos con un peor poder de predicción pueden obtener mejores puntuaciones de causalidad en sus contrafactuales. Es decir que la puntuación generada no debe arrojar confianza sobre el modelo que genera los contrafactuales, sino sobre los contrafactuales generados. Una puntuación causal elevada puede llevar a un efecto conocido en la literatura como \textit{explicabilidad de fachada}: una falsa sensación de que el modelo es válido porque sus contrafactuales son válidos.
...
Por diseño \texttt{CFGTS} no inspecciona los parámetros internos del modelo, sino que obtiene información del modelo mediante la ejecución repetida del modelo durante la fase de optimización de la Fase~1. Por ejemplo con la configuración experimental del Capítulo~\ref{cap:resultados} se dan hasta 300 evaluaciones del modelo en la Fase~1. Generar los contrafactuales y asignarles la puntuación es varios órdenes de magnitud más intensivo energéticamente que hacer una única inferencia. Esto se traduce en un mayor gasto eléctrico, y de forma indirecta mayor consumo de agua y mayores emisiones de dióxido de carbono. Este problema no es exclusivo de \texttt{CFGTS}, ya que otros métodos de explicabilidad como LIME o DiCE también sufren de esta desventaja.

Aun así, la implementación de referencia incluye mecanismos para acotar el impacto medioambiental mediante los parámetros \texttt{n\_trials}, \texttt{n\_coverage\_trials} y \texttt{timeout}, que permiten limitar tanto las evaluaciones como el tiempo máximo de búsqueda en los problemas de optimización. El operador que despliegue \texttt{CFGTS} puede intercambiar de forma consciente estabilidad de la explicación por consumo energético.

\section{Impacto ético}

De los capítulos de implementación y resultados se puede recordar que la puntuación causal es relativa a los contrafactuales. La presentación de estas puntuaciones a operadores no técnicos (como un médico, un paciente, un responsable de negocio o un regulador gubernamental) puede dar una apariencia de validez a modelos que no tienen porqué ser correctos. Esta confusión puede llevar a tomar decisiones con una confianza indebida. Es responsabilidad del operador de \texttt{CFGTS} comunicar esta distinción a los usuarios.

Segundo, el algoritmo admite un parámetro llamado \textit{whitelist} de variables modificables (Capítulo~\ref{cap:propuesta}). Este mecanismo puede usarse incorrectamente de la siguiente forma: imaginemos un modelo que deniega un préstamo por el género del cliente. El operador de \texttt{CFGTS} podría usar el algoritmo para generar un contrafactual en el que el género no está incluido en la \textit{whitelist}, y así poder justificar la decisión sesgada del modelo; se podrían justificar \textit{a posteriori} decisiones ya tomadas de forma discriminatoria.
...
\begin{itemize}
    \item \textbf{Fase 1: Investigación y fundamentos teóricos.} La fase 1 se dedicó a construir una base sólida de conocimiento en los campos necesarios para abordar el problema: estadística y probabilidad, causalidad \textit{pearliana} (grafos causales, modelos causales estructurales, intervenciones y mediación), series temporales (modelos autorregresivos, similaridad, clasificación) y el estado del arte en generación de explicaciones contrafactuales, tanto en datos tabulares como en series temporales. Esta fase destaca por dedicarse a revisar bibliografía especializada, mucha más de lo que originalmente se esperaba, y estudiar las limitaciones de los métodos existentes (en esencia el método \textit{LIME}) cuando se aplican a datos en series temporales. Se perfiló la motivación y el planteamiento del problema que después darían forma a los primeros capítulos de la memoria. Esta fase transcurrió aproximadamente desde enero de 2025 hasta septiembre de 2025, concentrándose especialmente en los primeros meses del proyecto.

    \item \textbf{Fase 2: Diseño y formalización matemática del algoritmo.} Una vez asentados la mayoría de fundamentos teóricos, se procedió a diseñar \texttt{CFGTS} como algoritmo de dos fases: generación de candidatos mediante optimización bi-objetivo y evaluación causal. Esta fase se centró en formalizar cada componente: las funciones objetivo $J_1$ y $J_2$, la noción de dominancia y optimalidad de Pareto, el grafo causal temporal y la puntuación de consistencia causal, así como la comparación entre el estimador basado en el DAG y una regresión lineal ajustada (LIME). El atento lector notará la mención de solo dos fases: el paso del algoritmo relativo a la generación de un conjunto de cobertura y el uso de la discrepancia estelar centrada $L^2$ se añadió después, al comprobar la falta de estabilidad numérica de las soluciones; un algoritmo sin implementar nunca hubiese podido mostrar la necesidad de este paso. De esta fase surgieron los enunciados y las demostraciones que conforman el Anexo B, un trabajo que exigió varias (en ocasiones demasiadas) iteraciones para refinar las hipótesis de cada teorema y evitar afirmaciones poco defendibles (por ejemplo acotar la dominancia del modelo causal al caso lineal). Esta fase se extendió aproximadamente desde mayo de 2025 hasta noviembre de 2025.

    \item \textbf{Fase 3: Implementación del paquete \texttt{cfgts}.} Esta fase consistió en materializar el algoritmo formalizado como un paquete de Python distribuible. Se desarrolló la clase principal \texttt{CFGTS}. También se incorporaron aspectos de calidad de software: tipado estático (\texttt{mypy}), pruebas automatizadas y la configuración del \textit{wheel} con \texttt{Hatchling}. Esta fase se desarrolló de forma intermitente entre julio de 2025 y abril de 2026, alternando periodos de programación intensiva con semanas de menor dedicación por coincidir con otras obligaciones o por necesitar refinar la parte teórica del algoritmo.

    \item \textbf{Fase 4: Experimentación y resultados.} Con el paquete ya publicado en \texttt{PyPI}, esta fase se dedicó a diseñar y ejecutar experimentos sobre conjuntos de datos sintéticos y reales, comparando la puntuación de consistencia causal obtenida mediante el modelo causal frente a una regresión lineal tipo LIME. También incluyó la elaboración de las figuras y tablas de resultados que se incorporan al capítulo correspondiente de la memoria. Se desarrolló entre marzo de 2026 y agosto de 2026.

    \item \textbf{Fase 5: Redacción de la memoria y revisión final.} La última fase se dedicó a la elaboración del documento \LaTeX completo. Esta fase se extendió desde enero de 2026 hasta la entrega final en septiembre de 2026. Conforme se materializaban ciertos resultados del algoritmo, se plasmaban directamente en la memoria.
\end{itemize}

En la Tabla~\ref{tab:planificacion} se resume la planificación descrita, indicando la fecha de inicio y fin estimadas de cada fase.

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Planificación del proyecto}\label{tab:planificacion}}
    {\begin{tabular}{|p{1.6cm}|p{6.4cm}|p{2.2cm}|p{2.2cm}|}
            \hline
            \textbf{Fase} & \textbf{Tarea realizada}                        & \textbf{Fecha inicio} & \textbf{Fecha fin} \\
            \hline
            ...
            \hline
            Fase 3        & Implementación del paquete \texttt{cfgts}       & 07/2025               & 04/2026            \\
            \hline
...
Esta capítulo contiene una estimación del coste tanto directo como indirecto de desarrollar el algoritmo desde el punto de vista de una empresa. Todas las cantidades se reflejan con IVA incluido y comprenden el período descrito en el Capítulo~\ref{cap:planificacion}.
...
\section{Coste de infraestructura}
...
El entorno de desarrollo se compone del IDE VSCode, Python, \texttt{uv}, Git, \LaTeX{} y las dependencias declaradas en el fichero \texttt{pyproject.toml}. Las bibliotecas principales de la implementación se han integrado respetando sus licencias de código abierto, por lo que no introducen un coste de licencia. La Tabla~\ref{tab:budget_resources} detalla estos recursos.

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Recursos materiales y de software}\label{tab:budget_resources}}
    {\begin{tabular}{|p{4.1cm}|p{5.2cm}|c|}
            \hline
            \textbf{Concepto}                  & \textbf{Notas}                                                             & \textbf{Coste total (€)} \\
            \hline
            Equipo de desarrollo y periféricos & Amortización: $\frac{21}{48} \times 1500$~€                                & 656{,}25                 \\
            \hline
            Software de desarrollo             & Python, VSCode, \texttt{uv}, Git, \LaTeX{} y bibliotecas de código abierto & 0{,}00                    \\
            \hline
            Servicios de computación externos  & No utilizados durante el desarrollo ni la evaluación                       & 0{,}00                    \\
            \hline
            \textbf{Total}                     &                                                                            & \textbf{656{,}25}        \\
            \hline
        \end{tabular}}
\end{table}
...
El mayor coste presupuestado se corresponde con el salario, por la parte de trabajo intelectual y técnico. Partiendo de la planificación en Capítulo~\ref{cap:planificacion}, se estima una dedicación total de 600 horas por parte del autor, que comprende todo el ciclo de vida del proyecto realizado. Se añaden 25 horas de dirección académica por parte del tutor.

La Tabla~\ref{tab:budget_staff} usa una tarifa de 15~€/hora para el autor, equivalente a un desarrollador junior en un contexto de beca académica, y de 50~€/hora para el tutor.

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Coste estimado del personal}\label{tab:budget_staff}}
    {\begin{tabular}{|p{4.3cm}|c|c|c|}
            \hline
            \textbf{Perfil}                                        & \textbf{Horas} & \textbf{Coste/hora (€)} & \textbf{Coste total (€)} \\
            \hline
            Autor: investigación, desarrollo, evaluación y memoria & 600            & 15{,}00                 & 9\,000{,}00              \\
            \hline
            Tutor: dirección académica y seguimiento               & 25             & 50{,}00                 & 1\,250{,}00              \\
            \hline
            \textbf{Total}                                         & \textbf{625}   &                         & \textbf{10\,250{,}00}    \\
            \hline
        \end{tabular}}
\end{table}
...
Los costes indirectos incluyen el consumo asociado al uso del equipo y a la conexión durante el proyecto. Para la electricidad se estima una potencia media de 60~W durante las 600 horas de dedicación \cite{apple_macbook_pro_m1_2020}. Según \cite{eurostat_electricity_2025} el precio medio de la electricidad con impuestos incluidos durante 2025 fue de 0{,}28565~€/kWh, lo que resulta en 10{,}28~€ de coste total. La conectividad se estima de forma prorrateada con un precio de 5~€ mensuales durante los 21 meses del proyecto.

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Costes indirectos}\label{tab:budget_indirect}}
    {\begin{tabular}{|p{5.2cm}|p{4.1cm}|c|}
            \hline
            \textbf{Concepto}       & \textbf{Criterio de valoración}                   & \textbf{Coste total (€)} \\
            \hline
            Electricidad            & $600$ h $\times$ {,}06 kW $\times$ {,}28565~€/kWh & 10{,}28                  \\
            \hline
            Conectividad a Internet & 21 meses $\times$ 5~€/mes                         & 105{,}00                 \\
            \hline
            \textbf{Total}          &                                                   & \textbf{115{,}28}        \\
            \hline
        \end{tabular}}
\end{table}

\section{Resumen y viabilidad}

La Tabla~\ref{tab:budget_total} resume todo el presupuesto. La dedicación de personal representa aproximadamente el 93\% del coste total, mientras que el software no añade costes adicionales.
\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Presupuesto total estimado del proyecto}\label{tab:budget_total}}
    {\begin{tabular}{|p{7.4cm}|c|}
            \hline
            \textbf{Partida}               & \textbf{Coste total (€)} \\
            \hline
            Recursos materiales y software & 656{,}25                 \\
            \hline
            Personal                       & 10\,250{,}00             \\
            \hline
            Costes indirectos              & 115{,}28                 \\
            \hline
            \textbf{Total estimado}        & \textbf{11\,021{,}53}    \\
            \hline
        \end{tabular}}
\end{table}
...
En este trabajo se ha desarrollado el algoritmo \texttt{CFGTS} junto con su implementación. El algoritmo combina la optimización bi-objetivo de candidatos cercanos al objetivo, la construcción de un conjunto de cobertura en el espacio de salida y la evaluación de la consistencia causal de los contrafactuales mediante un grafo temporal.
...
La implementación en Python materializa las tres fases del algoritmo en una API única y agnóstica respecto al regresor de \textit{scikit-learn}, e incorpora restricciones de modificabilidad.

La evaluación confirma que la puntuación depende de la naturaleza del modelo. Los modelos lineales alcanzan puntuaciones cercanas a la unidad, coherentes con la hipótesis de que sus cambios pueden describirse adecuadamente mediante efectos lineales. En cambio, los modelos \textit{Random Forest} muestran puntuaciones más bajas y dispersas, lo que refleja la dificultad de aproximar modelos no lineales.
...
\begin{description}
    \item[AR] \textit{AutoRegressive} (autorregresivo). Modelo en el que una variable se explica mediante sus propios valores retardados.
    \item[\textit{backdoor}] Criterio de ajuste por puerta trasera: conjunto de variables que, al condicionar (no intervenir) sobre ellas, bloquea los caminos de retroceso (confusores) entre una causa y un efecto, permitiendo identificar el efecto causal a partir de datos observacionales.
    \item[CDE] \textit{Controlled Direct Effect} (efecto directo controlado). Diferencial en el resultado al intervenir sobre la causa manteniendo fijo por intervención el mediador.
    \item[CFGTS] \textit{CounterFactual Generation for Time Series}. Algoritmo y paquete propuestos en este trabajo.
    \item[DAG] \textit{Directed Acyclic Graph} (grafo dirigido acíclico). Grafo cuyas aristas tienen dirección y en el que no existen ciclos.
    \item[DGP] \textit{Data Generating Process} (proceso generador de datos). Mecanismo, habitualmente desconocido, que produce datos observables.
    \item[DTW] \textit{Dynamic Time Warping}. Medida de similaridad elástica entre series temporales que permite estirar localmente una serie para mejorar su emparejamiento.
    \item[EDA] \textit{Exploratory Data Analysis} (análisis exploratorio de datos). Técnicas de explicabilidad aplicadas antes del uso de un modelo.
    \item[EDR] \textit{Edit Distance on Real sequence}. Medida de similaridad basada en edición entre series temporales.
    \item[ERP] \textit{Edit Distance with Real Penalty}. Medida de similaridad basada en edición entre series temporales con tolerancia a huecos.
    \item[\textit{fitted}] Cuando un modelo ha sido entrenado (ajustado) con los datos.
    \item[\textit{forward walking}] Recorrer el modelo autorregresivamente para generar predicciones adicionales.
    \item[GML] \textit{Graph Modeling Language}. Formato de representación de grafos.
    \item[LCSS] \textit{Longest Common Subsequence}. Medida de similaridad entre series temporales basada en la subsecuencia común más larga.
    \item[LIME] \textit{Local Interpretable Model-agnostic Explanations}. Método de referencia para generar modelos surrogados locales mediante perturbación y ponderación por cercanía.
    \item[MCE] Modelo Causal Estructural (véase SCM).
    \item[MCO] Mínimos Cuadrados Ordinarios. Método de estimación de los coeficientes de una regresión lineal minimizando la suma de residuos al cuadrado.
    \item[PNDE] \textit{Pure Natural Direct Effect} (efecto directo natural puro). Componente del efecto total que se transmite directamente de la causa al efecto.
    \item[RCT] \textit{Randomized Controlled Trial} (experimento controlado aleatorizado). Procedimiento experimental de referencia para la investigación causal.
    \item[SCM] \textit{Structural Causal Model} (modelo causal estructural). Conjunto de variables exógenas, endógenas y ecuaciones que determinan cómo se asignan los valores de las variables de interés.
    \item[\textit{shapelet}] Segmento característico de una serie temporal, utilizado por algunos métodos de clasificación y generación de contrafactuales.
    \item[sMAPE] \textit{symmetric Mean Absolute Percentage Error}. Métrica simétrica de error porcentual absoluto medio, usada para evaluar la calidad de la predicción recursiva de los modelos.
    \item[SVAR] \textit{Structural Vector AutoRegressive} (modelo autorregresivo estructural). Generalización del VAR que admite enlaces contemporáneos entre variables.
    \item[TE] Efecto Total (\textit{Total Effect}). Suma del efecto directo y de todos los efectos mediados entre una causa y un efecto.
    \item[TNIE] \textit{Total Natural Indirect Effect} (efecto indirecto natural total). Componente del efecto total que se transmite a través de la variable mediadora. Véase también PNDE.
    \item[TPE] \textit{Tree-structured Parzen Estimator}. Algoritmo de optimización bayesiana empleado por la biblioteca Optuna para resolver problemas de optimización.
    \item[VAR] \textit{Vector AutoRegressive} (vectorial autorregresivo). Modelo lineal matricial y multivariante donde cada variable se explica por todos los valores retardados de todas las variables del sistema.
    \item[\textit{whitelist}] Subconjunto de variables de entrada que el optimizador de \texttt{CFGTS} tiene permitido modificar al generar contrafactuales.
\end{description}

\chapter* {Anexo B. Demostraciones}
...
\section*{B.3 Grafo causal temporal}
...
\begin{itemize}
    \item $n_{\text{In}}, D \in \mathbb{N}$ con $D > 0$. Los vértices del grafo de \texttt{CFGTS} son los índices $\{0, \ldots, n_{\text{In}} + D - 1\}$: la entrada $X_i$ se identifica con el índice $i$ (de modo que $0 \leq i < n_{\text{In}}$) y la salida $Y_d$ con el índice $n_{\text{In}} + d$ (de modo que $n_{\text{In}} \leq n_{\text{In}} + d < n_{\text{In}} + D$).
    \item Una variable temporal se describe por un par $(\text{id}_{\text{base}}, \text{lag}) \in \mathbb{N} \times \mathbb{N}$. La relación de causalidad temporal $\mathrm{tf}_1 \xrightarrow{t} \mathrm{tf}_2$ equivale a $\mathrm{lag}(\mathrm{tf}_1) > \mathrm{lag}(\mathrm{tf}_2)$.
    \item $\mathbf{x}, \mathbf{x}' : \{0, \ldots, n_{\text{In}}-1\} \to \mathbb{R}$ denotan la instancia original y un candidato contrafactual, vistos como familias de coordenadas; $f_d : \mathbb{R}^{n_{\text{In}}} \to \mathbb{R}$ es la $d$-ésima salida del modelo ajustado.
    \item $\boldsymbol{\beta}^{(d)} \in \mathbb{R}^{n_{\text{In}}}$ es el vector de efectos causales estimados para la salida $d$; $\varepsilon > 0$ es la constante de regularización.
    \item Se define el cambio esperado y el cambio real por objetivo $d$:
          $$\Delta y_d^{\text{esp}}\!\left(\boldsymbol{\beta}^{(d)}\right) = \sum_{i=0}^{n_{\text{In}}-1} \beta_i^{(d)}(x'_i - x_i), \qquad \Delta y_d^{\text{real}} = f_d(\mathbf{x}') - f_d(\mathbf{x}),$$
          la puntuación de consistencia causal por objetivo
          $$s_d\!\left(\mathbf{x}'; \boldsymbol{\beta}^{(d)}\right) = 1 - \frac{\left|\Delta y_d^{\text{esp}} - \Delta y_d^{\text{real}}\right|}{\left|\Delta y_d^{\text{esp}}\right| + \left|\Delta y_d^{\text{real}}\right| + \varepsilon},$$
          y la puntuación media sobre las $D$ salidas
          $$s\!\left(\mathbf{x}'; \boldsymbol{\beta}\right) = \frac{1}{D}\sum_{d=0}^{D-1} s_d\!\left(\mathbf{x}'; \boldsymbol{\beta}^{(d)}\right).$$
    \item Cuando una salida $d$ y su vector $\boldsymbol{\beta}^{(d)}$ permanezcan fijos escribiremos simplemente $\Delta y_d^{\text{esp}}$ y $s_d(\mathbf{x}')$.
\end{itemize}

\paragraph*{Propiedades del grafo causal}

\paragraph*{Demostración del teorema ``Ausencia de auto-bucles en el grafo CFGTS''}
\begin{proof}
    Procedemos por contradicción. Supongamos que existe un vértice $v$ con $(v, v) \in E$. Por la definición de las aristas CFGTS, la pertenencia $(v,v) \in E$ se descompone en exactamente dos casos.

    \textit{Caso~(a) (arista entrada~$\to$~salida).} Existen $i \in \{0, \ldots, n_{\text{In}}-1\}$ y $d \in \{0, \ldots, D-1\}$ con $v = X_i$ y $v = Y_d$. Igualando los índices de un mismo vértice se obtiene $i = n_{\text{In}} + d$. Pero el índice de $X_i$ cumple $i < n_{\text{In}}$, mientras que el de $Y_d$ cumple $n_{\text{In}} \leq n_{\text{In}} + d$; encadenando ambas desigualdades,
    $$i < n_{\text{In}} \leq n_{\text{In}} + d = i,$$
    es decir, $i < i$, lo cual es imposible.

    \textit{Caso~(b) (arista temporal entrada~$\to$~entrada).} Existen índices $i \neq j$ en $\{0, \ldots, n_{\text{In}}-1\}$ con $v = X_i$ y $v = X_j$. Igualando los índices del vértice común se obtiene $i = j$, en contradicción directa con la hipótesis $i \neq j$.
...
\paragraph*{Demostración del teorema ``Irreflexividad de la causalidad temporal''}
\begin{proof}
    Supongamos, por contradicción, que $\mathrm{tf} \xrightarrow{t} \mathrm{tf}$. Por definición de la causalidad temporal, esto significa
    $$\mathrm{lag}(\mathrm{tf}) > \mathrm{lag}(\mathrm{tf}).$$
    Pero ningún número natural es estrictamente mayor que sí mismo. Contradicción. Por tanto, $\neg\,(\mathrm{tf} \xrightarrow{t} \mathrm{tf})$.
\end{proof}

\paragraph*{Demostración del teorema ``Asimetría de la causalidad temporal''}
\begin{proof}
    Supongamos que $\mathrm{tf}_1 \xrightarrow{t} \mathrm{tf}_2$, es decir, $\mathrm{lag}(\mathrm{tf}_1) > \mathrm{lag}(\mathrm{tf}_2)$. Esta desigualdad implica $\mathrm{lag}(\mathrm{tf}_2) < \mathrm{lag}(\mathrm{tf}_1)$, y en particular
    $$\mathrm{lag}(\mathrm{tf}_2) \ngtr \mathrm{lag}(\mathrm{tf}_1).$$
    Por tanto, $\neg\,(\mathrm{tf}_2 \xrightarrow{t} \mathrm{tf}_1)$.
\end{proof}

\paragraph*{Demostración del teorema ``Transitividad de la causalidad temporal''}
\begin{proof}
    Supongamos $\mathrm{tf}_1 \xrightarrow{t} \mathrm{tf}_2$ y $\mathrm{tf}_2 \xrightarrow{t} \mathrm{tf}_3$, es decir,
    $$\mathrm{lag}(\mathrm{tf}_1) > \mathrm{lag}(\mathrm{tf}_2) \qquad \text{y} \qquad \mathrm{lag}(\mathrm{tf}_2) > \mathrm{lag}(\mathrm{tf}_3).$$
    Por transitividad del orden estricto sobre $\mathbb{N}$,
    $$\mathrm{lag}(\mathrm{tf}_1) > \mathrm{lag}(\mathrm{tf}_3),$$
    es decir, $\mathrm{tf}_1 \xrightarrow{t} \mathrm{tf}_3$.
\end{proof}

\paragraph*{Propiedades de la whitelist}

\paragraph*{Demostración del teorema ``Whitelist universal''}
\begin{proof}
    Sea $W = \{0, \ldots, n_{\text{In}}-1\}$ y $\mathbf{x}'$ cualquier candidato contrafactual. Para todo índice $i$ se tiene $i \in W$, de modo que no existe ningún $i \notin W$. La condición de respeto a la \textit{whitelist}, que exige $x'_i = x_i$ para todo $i \notin W$, se satisface vacuamente. Por tanto, todo $\mathbf{x}'$ respeta $W$.
\end{proof}

\paragraph*{Demostración del teorema ``Reflexividad del respeto a la whitelist''}
\begin{proof}
    Sea $\mathbf{x}' = \mathbf{x}$ y $W$ cualquier \textit{whitelist}. Para todo $i \notin W$, se tiene $x'_i = x_i$ porque $\mathbf{x}' = \mathbf{x}$. Por tanto, $\mathbf{x}$ respeta $W$ respecto de sí mismo.
\end{proof}

\paragraph*{Propiedades de la puntuación causal por objetivo}

Para simplificar la escritura, fijamos una salida $d$ y denotamos $a = \Delta y_d^{\text{esp}}$ y $b = \Delta y_d^{\text{real}}$, de modo que
$$s_d(\mathbf{x}') = 1 - \frac{|a - b|}{|a| + |b| + \varepsilon}.$$

\paragraph*{Demostración del teorema ``Cota superior por objetivo''}
\begin{proof}
    El numerador satisface $|a - b| \geq 0$ y el denominador cumple $|a| + |b| + \varepsilon \geq \varepsilon > 0$. Por tanto, la fracción es no negativa y
    $$s_d(\mathbf{x}') = 1 - \frac{|a - b|}{|a| + |b| + \varepsilon} \leq 1 - 0 = 1.$$
\end{proof}

\paragraph*{Demostración del teorema ``Positividad estricta por objetivo''}
\begin{proof}
    Probamos primero la desigualdad triangular $|a - b| \leq |a| + |b|$ distinguiendo el signo de $a - b$:
    \begin{itemize}
        \item Si $a - b \leq 0$, entonces $|a - b| = -(a - b) = b - a$. Usando $b \leq |b|$ y $-a \leq |a|$ se obtiene $b - a \leq |a| + |b|$.
        \item Si $a - b > 0$, entonces $|a - b| = a - b$. Usando $a \leq |a|$ y $-b \leq |b|$ se obtiene $a - b \leq |a| + |b|$.
    \end{itemize}
    En ambos casos $|a - b| \leq |a| + |b|$. El denominador es estrictamente positivo, $|a| + |b| + \varepsilon > 0$; sumando $\varepsilon > 0$ a la cota anterior,
    $$|a - b| \leq |a| + |b| < |a| + |b| + \varepsilon.$$
    Como el numerador es estrictamente menor que un denominador positivo, el cociente es menor que la unidad,
    $$\frac{|a - b|}{|a| + |b| + \varepsilon} < 1,$$
    y por tanto $s_d(\mathbf{x}') = 1 - \dfrac{|a - b|}{|a| + |b| + \varepsilon} > 0$.
\end{proof}

\paragraph*{Demostración del teorema ``Puntuación máxima por alineación perfecta''}
\begin{proof}
    Si $a = b$, entonces $a - b = 0$, y por tanto $|a - b| = 0$. En consecuencia,
    $$s_d(\mathbf{x}') = 1 - \frac{0}{|a| + |b| + \varepsilon} = 1 - 0 = 1.$$
\end{proof}

\paragraph*{Demostración del teorema ``Puntuación de la instancia original''}
\begin{proof}
    Si $\mathbf{x}' = \mathbf{x}$, entonces $x'_i - x_i = 0$ para todo $i \in \{0, \ldots, n_{\text{In}}-1\}$. Por tanto,
    $$a = \Delta y_d^{\text{esp}} = \sum_{i=0}^{n_{\text{In}}-1} \beta_i^{(d)} \cdot 0 = 0.$$
    Análogamente, $b = \Delta y_d^{\text{real}} = f_d(\mathbf{x}) - f_d(\mathbf{x}) = 0$. Como $a = b = 0$, por el teorema de puntuación máxima por alineación perfecta, $s_d(\mathbf{x}') = 1$.
\end{proof}

\paragraph*{Demostración del teorema ``Desalineación implica puntuación sub-óptima''}
\begin{proof}
    Si $a \neq b$, entonces $a - b \neq 0$, y por ser el valor absoluto de un real no nulo, $|a - b| > 0$. El denominador satisface $|a| + |b| + \varepsilon > 0$. En consecuencia, el cociente es estrictamente positivo y
    $$s_d(\mathbf{x}') = 1 - \frac{|a - b|}{|a| + |b| + \varepsilon} < 1.$$
\end{proof}

\paragraph*{Propiedades de la puntuación media}

\paragraph*{Demostración del teorema ``Cota superior de la puntuación media''}
\begin{proof}
    Para cada salida $d \in \{0, \ldots, D-1\}$, el teorema de cota superior por objetivo garantiza $s_d(\mathbf{x}') \leq 1$. Sumando sobre todas las salidas,
    $$\sum_{d=0}^{D-1} s_d(\mathbf{x}') \leq \sum_{d=0}^{D-1} 1 = D.$$
    Dividiendo por $D > 0$, se concluye $s(\mathbf{x}') = \dfrac{1}{D}\sum_{d=0}^{D-1} s_d(\mathbf{x}') \leq 1$.
\end{proof}

\paragraph*{Demostración del teorema ``Puntuación media de la instancia original''}
\begin{proof}
    Si $\mathbf{x}' = \mathbf{x}$, el teorema de puntuación de la instancia original garantiza $s_d(\mathbf{x}') = 1$ para todo $d \in \{0, \ldots, D-1\}$. Por tanto,
    $$\sum_{d=0}^{D-1} s_d(\mathbf{x}') = D,$$
    y como $D > 0$, $s(\mathbf{x}') = D/D = 1$.
\end{proof}

\paragraph*{Demostración del teorema ``Positividad de la puntuación media''}
\begin{proof}
    Por el teorema de positividad estricta por objetivo, cada sumando es estrictamente positivo: $s_d(\mathbf{x}') > 0$ para todo $d \in \{0, \ldots, D-1\}$. Como $D > 0$, el índice $d = 0$ está bien definido y $s_0(\mathbf{x}') > 0$. Al ser todos los sumandos no negativos, un único término acota inferiormente la suma,
    $$0 < s_0(\mathbf{x}') \leq \sum_{d=0}^{D-1} s_d(\mathbf{x}'),$$
    de modo que el numerador es estrictamente positivo. Como además el denominador $D$ es positivo, el cociente de dos cantidades positivas es positivo:
    $$s(\mathbf{x}') = \frac{1}{D}\sum_{d=0}^{D-1} s_d(\mathbf{x}') > 0.$$
\end{proof}

\paragraph*{Propiedades del cambio esperado}

\paragraph*{Demostración del teorema ``Linealidad del cambio esperado''}
\begin{proof}
    Por hipótesis, $x''_i - x_i = (x'_i - x_i) + (x''_i - x'_i)$ para todo $i$. Entonces,
    \begin{align*}
        \Delta y_d^{\text{esp}}(\mathbf{x}, \mathbf{x}'')
         & = \sum_{i=0}^{n_{\text{In}}-1} \beta_i^{(d)} \cdot (x''_i - x_i)                                                      \\
         & = \sum_{i=0}^{n_{\text{In}}-1} \beta_i^{(d)} \cdot \bigl[(x'_i - x_i) + (x''_i - x'_i)\bigr]                          \\
         & = \sum_{i=0}^{n_{\text{In}}-1} \beta_i^{(d)} (x'_i - x_i) + \sum_{i=0}^{n_{\text{In}}-1} \beta_i^{(d)} (x''_i - x'_i) \\
         & = \Delta y_d^{\text{esp}}(\mathbf{x}, \mathbf{x}') + \Delta y_d^{\text{esp}}(\mathbf{x}', \mathbf{x}''),
    \end{align*}
    donde se ha usado la distributividad de la multiplicación respecto de la suma.
\end{proof}

\paragraph*{Demostración del teorema ``Cambio esperado nulo fuera del soporte''}
\begin{proof}
    Sea $S \subseteq \{0, \ldots, n_{\text{In}}-1\}$ tal que $\beta_i^{(d)} = 0$ para $i \notin S$ y $x'_i = x_i$ para $i \in S$. Para cada índice $i$ se dan dos casos:
    \begin{itemize}
        \item Si $i \in S$: $x'_i - x_i = 0$, luego $\beta_i^{(d)} \cdot (x'_i - x_i) = 0$.
...
\paragraph*{Demostración del teorema ``Reducción por whitelist''}
\begin{proof}
    Como $W \subseteq \{0, \ldots, n_{\text{In}}-1\}$, basta comprobar que los términos del índice completo que no están en $W$ no aportan nada a la suma. En efecto, si $\mathbf{x}'$ respeta la \textit{whitelist} $W$, para todo $i \notin W$ se tiene $x'_i = x_i$, luego
    $$\beta_i^{(d)} \cdot (x'_i - x_i) = \beta_i^{(d)} \cdot 0 = 0.$$
    Por consiguiente, restringir la suma sobre la familia completa de índices al subconjunto $W$ no altera su valor:
    $$\Delta y_d^{\text{esp}} = \sum_{i=0}^{n_{\text{In}}-1} \beta_i^{(d)} \cdot (x'_i - x_i) = \sum_{i \in W} \beta_i^{(d)} \cdot (x'_i - x_i).$$
\end{proof}

\paragraph*{Propiedades para modelos lineales}

Para esta parte fijamos una salida $d$ y suponemos que $f_d$ es lineal con pesos $\mathbf{w}^{(d)} \in \mathbb{R}^{n_{\text{In}}}$ e intercepto $b_d \in \mathbb{R}$:
$$f_d(\mathbf{x}) = \sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} x_i + b_d.$$
Recordamos asimismo el error de predicción entre dos vectores de coeficientes $\boldsymbol{\gamma}, \boldsymbol{\theta} \in \mathbb{R}^{n_{\text{In}}}$,
$$E_d(\boldsymbol{\gamma}, \boldsymbol{\theta}, \mathbf{x}, \mathbf{x}') = \sum_{i=0}^{n_{\text{In}}-1} (\gamma_i - \theta_i)(x'_i - x_i),$$
y la noción de consistencia con el DAG respecto del conjunto de padres $P \subseteq \{0, \ldots, n_{\text{In}}-1\}$: el vector $\boldsymbol{\beta}^{(d)}$ es consistente con $(P, \mathbf{w}^{(d)})$ si $\beta_i^{(d)} = w_i^{(d)}$ para $i \in P$ y $\beta_i^{(d)} = 0$ para $i \notin P$.

\paragraph*{Demostración del teorema ``Coincidencia de deltas para modelo lineal''}
\begin{proof}
    Partimos de la definición del cambio real y desarrollamos la forma lineal de $f_d$:
    \begin{align*}
        \Delta y_d^{\text{real}}
         & = f_d(\mathbf{x}') - f_d(\mathbf{x})                                                                                             \\
         & = \left(\sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} x'_i + b_d\right) - \left(\sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} x_i + b_d\right) \\
         & = \sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} x'_i - \sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} x_i                                       \\
         & = \sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} (x'_i - x_i),
    \end{align*}
    donde los interceptos $b_d$ se cancelan y los sumatorios se combinan término a término. Por otro lado, la definición del cambio esperado junto con la hipótesis $\boldsymbol{\beta}^{(d)} = \mathbf{w}^{(d)}$ da
    $$\Delta y_d^{\text{esp}} = \sum_{i=0}^{n_{\text{In}}-1} \beta_i^{(d)} (x'_i - x_i) = \sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} (x'_i - x_i).$$
    Ambas expresiones son idénticas, luego $\Delta y_d^{\text{esp}} = \Delta y_d^{\text{real}}$.
\end{proof}

\paragraph*{Demostración del teorema ``Puntuación perfecta para modelo lineal calibrado''}
\begin{proof}
    Escribimos $a = \Delta y_d^{\text{esp}}$ y $b = \Delta y_d^{\text{real}}$. Por el teorema de coincidencia de deltas, como $\boldsymbol{\beta}^{(d)} = \mathbf{w}^{(d)}$ y $f_d$ es lineal, se tiene $a = b$. Sustituyendo en la definición de la puntuación, el numerador es $|a - b| = 0$ y el denominador $|a| + |b| + \varepsilon > 0$, de modo que
    $$s_d(\mathbf{x}') = 1 - \frac{|a - b|}{|a| + |b| + \varepsilon} = 1 - \frac{0}{|a| + |b| + \varepsilon} = 1.$$
\end{proof}

\paragraph*{Demostración del teorema ``Descomposición de la discrepancia''}
\begin{proof}
    Por el teorema de coincidencia de deltas, el cambio real admite la forma lineal $\Delta y_d^{\text{real}} = \sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} (x'_i - x_i)$. Restando esta expresión de la definición del cambio esperado para $\boldsymbol{\beta}^{(d)}$ y agrupando término a término,
    \begin{align*}
        \Delta y_d^{\text{esp}}\!\left(\boldsymbol{\beta}^{(d)}\right) - \Delta y_d^{\text{real}}
         & = \sum_{i=0}^{n_{\text{In}}-1} \beta_i^{(d)} (x'_i - x_i) - \sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} (x'_i - x_i) \\
         & = \sum_{i=0}^{n_{\text{In}}-1} \bigl(\beta_i^{(d)} - w_i^{(d)}\bigr) (x'_i - x_i)                               \\
         & = E_d(\boldsymbol{\beta}^{(d)}, \mathbf{w}^{(d)}, \mathbf{x}, \mathbf{x}'),
    \end{align*}
    donde la última igualdad es exactamente la definición del error de predicción.
\end{proof}

\paragraph*{Demostración del teorema ``Error nulo con efectos consistentes con el DAG''}
\begin{proof}
    Sea $P$ el conjunto de padres. Por las hipótesis de consistencia con el DAG y de dispersión del modelo ($w_i^{(d)} = 0$ para $i \notin P$), para cada índice $i$:
    \begin{itemize}
        \item Si $i \in P$: $\beta_i^{(d)} = w_i^{(d)}$ por la condición de coincidencia en padres, luego $\beta_i^{(d)} - w_i^{(d)} = 0$.
        \item Si $i \notin P$: $\beta_i^{(d)} = 0$ por la condición de cero fuera de padres, y $w_i^{(d)} = 0$ por hipótesis, luego $\beta_i^{(d)} - w_i^{(d)} = 0$.
    \end{itemize}
    Como todos los factores $(\beta_i^{(d)} - w_i^{(d)})$ son nulos,
    $$E_d(\boldsymbol{\beta}^{(d)}, \mathbf{w}^{(d)}, \mathbf{x}, \mathbf{x}') = \sum_{i=0}^{n_{\text{In}}-1} (\beta_i^{(d)} - w_i^{(d)})(x'_i - x_i) = 0.$$
\end{proof}

\paragraph*{Demostración del teorema ``Puntuación máxima con efectos consistentes con el DAG''}
\begin{proof}
    Escribimos $a = \Delta y_d^{\text{esp}}$ y $b = \Delta y_d^{\text{real}}$. Por el teorema anterior el error de predicción es nulo, $E_d = 0$, y por el teorema de descomposición de la discrepancia, $a - b = E_d = 0$, esto es, $a = b$. Sustituyendo en la definición de la puntuación, el numerador $|a - b| = 0$ produce
    $$s_d(\mathbf{x}') = 1 - \frac{0}{|a| + |b| + \varepsilon} = 1.$$
\end{proof}

\paragraph*{Demostración del teorema ``Error no nulo implica puntuación sub-óptima''}
\begin{proof}
    Escribimos $a = \Delta y_d^{\text{esp}}$ y $b = \Delta y_d^{\text{real}}$. Por el teorema de descomposición de la discrepancia, $a - b = E_d$; como por hipótesis $E_d \neq 0$, se tiene $a \neq b$, luego $a - b \neq 0$ y $|a - b| > 0$. El denominador cumple $|a| + |b| + \varepsilon \geq \varepsilon > 0$, de modo que el cociente de un numerador positivo entre un denominador positivo es estrictamente positivo y
    $$s_d(\mathbf{x}') = 1 - \frac{|a - b|}{|a| + |b| + \varepsilon} < 1.$$
\end{proof}

\paragraph*{Demostración del teorema ``Dominancia del DAG sobre efectos mal calibrados''}
\begin{proof}
    Para los efectos $\boldsymbol{\beta}_{\text{dag}}$, consistentes con el DAG, el teorema de error nulo da $E_d = 0$; por el teorema de descomposición de la discrepancia, $\Delta y_d^{\text{esp}} = \Delta y_d^{\text{real}}$, y el numerador nulo de la puntuación da $s_{d, \boldsymbol{\beta}_{\text{dag}}}(\mathbf{x}') = 1$. Para $\boldsymbol{\beta}_{\text{otro}}$, cuyo error de predicción respecto de $f_d$ es no nulo, el mismo teorema de descomposición da $\Delta y_d^{\text{esp}} \neq \Delta y_d^{\text{real}}$, luego $|\Delta y_d^{\text{esp}} - \Delta y_d^{\text{real}}| > 0$ y la puntuación es estrictamente menor que la unidad, $s_{d, \boldsymbol{\beta}_{\text{otro}}}(\mathbf{x}') < 1$. Combinando ambas,
    $$s_{d, \boldsymbol{\beta}_{\text{otro}}}(\mathbf{x}') < 1 = s_{d, \boldsymbol{\beta}_{\text{dag}}}(\mathbf{x}').$$
\end{proof}

\paragraph*{Efectos dinámicos totales y sesgo de mediación}

Para las demostraciones de esta parte fijamos $p, n \in \mathbb{N}$ con $n > 0$, un sistema de retardos $A : \{1, \ldots, p\} \to \mathbb{R}^{n \times n}$ y la convención $A_q = 0$ para $q > p$. El efecto total está definido por $\mathbf{T}^{(0)} = I_n$ y
$$\mathbf{T}^{(h+1)} = \sum_{j=0}^{h} A_{j+1}\,\mathbf{T}^{(h-j)}.$$
El efecto directo y el mediado son
$$\mathbf{D}^{(h+1)} = A_{h+1}, \qquad \mathbf{M}^{(h+1)} = \sum_{j=0}^{h-1} A_{j+1}\,\mathbf{T}^{(h-j)}.$$

\paragraph*{Demostración del teorema ``Descomposición en efecto directo y mediado''}
\begin{proof}
    Por la definición recursiva,
    $$\mathbf{T}^{(h+1)} = \sum_{j=0}^{h} A_{j+1}\,\mathbf{T}^{(h-j)}.$$
    Separamos el término correspondiente a $j = h$:
    $$\mathbf{T}^{(h+1)} = A_{h+1}\,\mathbf{T}^{(h-h)} + \sum_{j=0}^{h-1} A_{j+1}\,\mathbf{T}^{(h-j)}.$$
    Como $h - h = 0$ y $\mathbf{T}^{(0)} = I_n$, el primer sumando es $A_{h+1} \cdot I_n = A_{h+1} = \mathbf{D}^{(h+1)}$. El segundo sumando es precisamente $\mathbf{M}^{(h+1)}$ por definición. Por tanto,
    $$\mathbf{T}^{(h+1)} = \mathbf{D}^{(h+1)} + \mathbf{M}^{(h+1)}.$$
\end{proof}

\paragraph*{Demostración del teorema ``Sesgo del MCO ajustado''}
\begin{proof}
    Por definición del sesgo,
    $$\mathbf{B}_{\mathrm{MCO}}^{(h+1)} = \mathbf{D}^{(h+1)} - \mathbf{T}^{(h+1)}.$$
    Sustituyendo el resultado del teorema de descomposición,
    $$\mathbf{B}_{\mathrm{MCO}}^{(h+1)} = \mathbf{D}^{(h+1)} - \bigl(\mathbf{D}^{(h+1)} + \mathbf{M}^{(h+1)}\bigr) = -\mathbf{M}^{(h+1)}.$$
\end{proof}

\paragraph*{CausalModel vs Naive LIME por objetivo}

En esta parte fijamos una salida $d$ y suponemos que $f_d$ es lineal con vector de coeficientes igual al efecto total $\boldsymbol{\tau}^{(d)} \in \mathbb{R}^{n_{\text{In}}}$ e intercepto $b_d$; es decir, los pesos internos del modelo son $\mathbf{w}^{(d)} = \boldsymbol{\tau}^{(d)}$. Se comparan los dos estimadores
$$\widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}} = \boldsymbol{\tau}^{(d)} + \mathbf{b}^{(d)}, \qquad \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}} = \boldsymbol{\tau}^{(d)},$$
donde $\mathbf{b}^{(d)} \in \mathbb{R}^{n_{\text{In}}}$ es el vector de sesgo del MCO ajustado.

\paragraph*{Demostración del teorema ``Error de predicción del MCO''}
\begin{proof}
    Por definición, $\widehat{\boldsymbol{\beta}}_{\text{mco}}^{(d)} = \boldsymbol{\tau}^{(d)} + \mathbf{b}^{(d)}$. Para cada índice $i$,
    $$\widehat{\beta}_{i,\text{mco}}^{(d)} - \tau_i^{(d)} = \tau_i^{(d)} + b_i^{(d)} - \tau_i^{(d)} = b_i^{(d)}.$$
    En consecuencia,
    \begin{align*}
        E_d\!\left(\widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}}, \boldsymbol{\tau}^{(d)}, \mathbf{x}, \mathbf{x}'\right)
         & = \sum_{i=0}^{n_{\text{In}}-1} \bigl(\widehat{\beta}_{i,\text{mco}}^{(d)} - \tau_i^{(d)}\bigr)(x'_i - x_i) \\
         & = \sum_{i=0}^{n_{\text{In}}-1} b_i^{(d)} \cdot (x'_i - x_i).
    \end{align*}
\end{proof}

\paragraph*{Demostración del teorema ``Error nulo del backdoor''}
\begin{proof}
    Por definición, $\widehat{\boldsymbol{\beta}}_{\text{causal}}^{(d)} = \boldsymbol{\tau}^{(d)}$. Para cada índice $i$,
    $$\widehat{\beta}_{i,\text{causal}}^{(d)} - \tau_i^{(d)} = \tau_i^{(d)} - \tau_i^{(d)} = 0.$$
    Por tanto,
    $$E_d\!\left(\widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}, \boldsymbol{\tau}^{(d)}, \mathbf{x}, \mathbf{x}'\right) = \sum_{i=0}^{n_{\text{In}}-1} 0 \cdot (x'_i - x_i) = 0.$$
\end{proof}

\paragraph*{Demostración del teorema ``Puntuación máxima del CausalModel''}
\begin{proof}
    El estimador causal coincide con los coeficientes internos del modelo, $\widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}} = \boldsymbol{\tau}^{(d)} = \mathbf{w}^{(d)}$. Calculando el cambio esperado desde su definición y usando la forma lineal del cambio real,
    $$\Delta y_d^{\text{esp}}\!\left(\widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right) = \sum_{i=0}^{n_{\text{In}}-1} \tau_i^{(d)} (x'_i - x_i) = \sum_{i=0}^{n_{\text{In}}-1} w_i^{(d)} (x'_i - x_i) = \Delta y_d^{\text{real}}.$$
    Con $a = \Delta y_d^{\text{esp}}$ y $b = \Delta y_d^{\text{real}}$, esto es $a = b$; el numerador de la puntuación es $|a - b| = 0$ y, en consecuencia,
    $$s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right) = 1 - \frac{0}{|a| + |b| + \varepsilon} = 1.$$
\end{proof}

\paragraph*{Demostración del teorema ``Dominancia estricta: perturbación de una sola variable''}
\begin{proof}
    Por hipótesis, $x'_i = x_i$ para todo $i \neq j$, con $x'_j \neq x_j$ y $b_j^{(d)} \neq 0$. Por el teorema de error de predicción del MCO,
    $$E_d = \sum_{i=0}^{n_{\text{In}}-1} b_i^{(d)} (x'_i - x_i).$$
    Como $x'_i - x_i = 0$ salvo para $i = j$, todos los sumandos se anulan excepto ese, de modo que $E_d = b_j^{(d)} (x'_j - x_j)$. Al ser producto de dos factores no nulos, $E_d \neq 0$. Por el teorema de descomposición de la discrepancia, $\Delta y_d^{\text{esp}} - \Delta y_d^{\text{real}} = E_d \neq 0$, luego $|\Delta y_d^{\text{esp}} - \Delta y_d^{\text{real}}| > 0$ y
    $$s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}}\right) < 1 = s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right),$$
    donde la igualdad de la derecha es el teorema de puntuación máxima del CausalModel.
\end{proof}

\paragraph*{Demostración del teorema ``Dominancia estricta: contrafactual general''}
\begin{proof}
    Por hipótesis, $\sum_{i=0}^{n_{\text{In}}-1} b_i^{(d)} (x'_i - x_i) \neq 0$. Por el teorema de error de predicción del MCO, el error es exactamente $E_d = \sum_{i=0}^{n_{\text{In}}-1} b_i^{(d)} (x'_i - x_i)$, luego $E_d \neq 0$. Por el teorema de descomposición de la discrepancia, $\Delta y_d^{\text{esp}} - \Delta y_d^{\text{real}} = E_d \neq 0$, de donde $|\Delta y_d^{\text{esp}} - \Delta y_d^{\text{real}}| > 0$ y
    $$s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}}\right) < 1 = s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right),$$
    donde la igualdad de la derecha es el teorema de puntuación máxima del CausalModel.
\end{proof}

\paragraph*{Demostración del teorema ``Dominancia débil: siempre''}
\begin{proof}
    ...
    $$s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}}\right) = 1 - \frac{|\Delta y_d^{\text{esp}} - \Delta y_d^{\text{real}}|}{|\Delta y_d^{\text{esp}}| + |\Delta y_d^{\text{real}}| + \varepsilon} \leq 1 = s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right).$$
\end{proof}

\paragraph*{Demostración del teorema ``Condición necesaria y suficiente de igualdad''}
\begin{proof}
    ...
    \noindent$(\Rightarrow)$ Supongamos $s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}}\right) = 1$ y, por reducción al absurdo, que $\sum_{i} b_i^{(d)} (x'_i - x_i) \neq 0$. Por el teorema de error de predicción del MCO, $E_d = \sum_{i} b_i^{(d)} (x'_i - x_i) \neq 0$, y por el teorema de descomposición de la discrepancia, $a - b = E_d \neq 0$. Entonces $|a - b| > 0$, la fracción es estrictamente positiva y $s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}}\right) < 1$, en contradicción con la hipótesis. Por tanto,
    $$\sum_{i=0}^{n_{\text{In}}-1} b_i^{(d)} (x'_i - x_i) = 0.$$

    \noindent$(\Leftarrow)$ Supongamos que $\sum_{i} b_i^{(d)} (x'_i - x_i) = 0$. Por el teorema de error de predicción del MCO, $E_d = 0$, y por el teorema de descomposición de la discrepancia, $a - b = E_d = 0$, esto es $a = b$. Sustituyendo en la puntuación, $|a - b| = 0$ y
    $$s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}}\right) = 1 - \frac{0}{|a| + |b| + \varepsilon} = 1 = s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right).$$
\end{proof}

\paragraph*{Resultados agregados multi-salida}

\paragraph*{Demostración del teorema ``Puntuación media máxima del CausalModel''}
\begin{proof}
    Por el teorema de puntuación máxima del CausalModel, $s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right) = 1$ para cada $d \in \{0, \ldots, D-1\}$. Sustituyendo en la definición de la puntuación media,
    $$s\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}_{\text{causal}}\right) = \frac{1}{D}\sum_{d=0}^{D-1} s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right) = \frac{1}{D}\sum_{d=0}^{D-1} 1 = \frac{D}{D} = 1,$$
    donde la última igualdad usa $D > 0$.
\end{proof}

\paragraph*{Demostración del teorema ``Dominancia débil agregada del CausalModel''}
\begin{proof}
    Por el teorema de dominancia débil, para cada salida $d \in \{0, \ldots, D-1\}$,
    $$s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}}\right) \leq s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right).$$
    Sumando estas $D$ desigualdades término a término,
    $$\sum_{d=0}^{D-1} s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{mco}}\right) \leq \sum_{d=0}^{D-1} s_d\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}^{(d)}_{\text{causal}}\right).$$
    Multiplicando ambos miembros por $\tfrac{1}{D} > 0$, que preserva el sentido de la desigualdad, y aplicando la definición de la puntuación media,
    $$s\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}_{\text{mco}}\right) \leq s\!\left(\mathbf{x}'; \widehat{\boldsymbol{\beta}}_{\text{causal}}\right).$$
\end{proof}
...
\chapter* {Anexo C. Declaración de uso de IA Generativa en el Trabajo de Fin de Grado}
...
\chapter* {Anexo D. Declaración de originalidad}
