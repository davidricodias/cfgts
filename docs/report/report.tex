...
\begin{definition}[Sistema dinámico lineal de orden $p$]
    Sea $n \in \mathbb{N}$ el número de variables observadas. Un sistema dinámico lineal con $p$ retardos se escribe como
    \begin{equation}
        \mathbf{Z}_t = A_1 \mathbf{Z}_{t-1} + A_2 \mathbf{Z}_{t-2} + \cdots + A_p \mathbf{Z}_{t-p} + \boldsymbol{\varepsilon}_t,
...
\begin{definition}[Efecto total dinámico por horizonte]
    ...
    \end{equation}
    con la convención $A_q = 0$ cuando $q > p$. La matriz $\mathbf{T}^{(h+1)}$ agrega \textbf{todas} las trayectorias dirigidas que conectan el instante $t-(h+1)$ con el instante $t$.
\end{definition}
